...
void INC_MULTI_SET_CHANNEL(INC_UINT8 ucI2CID, ST_SUBCH_INFO* pMultiInfo)
{
  ...
  if(pMultiInfo->astSubChInfo[0].ucServiceType==3)//���� �׻� pMultiInfo->nSetCnt=1, uiLoop=0�̴�
  {
...
INC_UINT8 INC_INIT(INC_UINT8 ucI2CID)
{
  ...
  if( m_ucPLL_Mode == INPUT_CLOCK_24576KHZ) INC_CMD_WRITE(ucI2CID, APB_GEN_BASE + 0x10, 0x0162);
  else if(m_ucPLL_Mode == INPUT_CLOCK_19200KHZ) INC_CMD_WRITE(ucI2CID, APB_GEN_BASE + 0x10, 0x0422);
  INC_DELAY(10);
...
//110127 (EBI2 : HW AutoResync ������ EBI2 I/F ���� �߻�, TSIF : OK)
  ...
  INC_CMD_WRITE(ucI2CID, APB_PHY_BASE+ 0xEB, 0x0037);
  INC_CMD_WRITE(ucI2CID, APB_DEINT_BASE+ 0x01, 0x0001);   // Deinterleaver Enable ('10. 11. 04) - ��ġ ����
...
#define SYNC_OFFSET		0x4
#define SYNC_REF		0x100
#define RF_LEVEL_OFFSET	(0x10)
INC_UINT8 INC_SYNCDETECTOR(INC_UINT8 ucI2CID, INC_UINT32 ulFreq, INC_UINT8 ucScanMode)
{
	INC_UINT16		wOperState;//, wNullFlag  = 0;
	ST_BBPINFO* 	pInfo;
	INC_UINT16		awData[2], uiTimeOut = 0;
	INC_UINT8		ucIfLevel, ucRfLevel, ucLevelTick = 0;//ucLoop, 
	INC_UINT16		wOperStateStatus = 0x0;
	...
	while(1) {
		...
		ucRfLevel = INC_CMD_READ(ucI2CID, APB_RF_BASE+ 0xD1) & 0x3F; 
		ucIfLevel = INC_CMD_READ(ucI2CID, APB_RF_BASE+ 0xD2) & 0x3F;

		awData[0] = INC_CMD_READ(ucI2CID, APB_PHY_BASE+ 0x28); 
		awData[1] = INC_CMD_READ(ucI2CID, APB_PHY_BASE+ 0x8A); 

		wOperState = INC_CMD_READ(ucI2CID, APB_PHY_BASE+ 0x10);
		wOperState = ((wOperState & 0x7000) >> 12);

		if(wOperState >= 3) 	wOperStateStatus = wOperState;

		//TDMB_MSG_INC_BB("\r\n ========== [%d][%d] [0x%.4X][0x%.4X] :: [0x%.2X][0x%.2X]",ulFreq, wOperState, awData[0], awData[1],
		//	ucRfLevel, ucIfLevel);
		...
		if(wOperStateStatus == 0){
			if(awData[0] == 0 && awData[1] == 0){
				pInfo->nBbpStatus = ERROR_SYNC_NO_SIGNAL+1;
				break;
			}
		}
		if(ucRfLevel >= 0x3f && ucIfLevel >= 0x37){
			pInfo->nBbpStatus = ERROR_SYNC_NO_SIGNAL+2;
			break;
		}
		...
		if(uiTimeOut >= (2500 / 20)){
			pInfo->nBbpStatus = ERROR_SYNC_TIMEOUT;
			break;
		}

		if(ucRfLevel > 0x20){
			if(abs(awData[1] - SYNC_REF) >= SYNC_OFFSET && awData[1])	ucLevelTick++;
			if(abs(awData[0] - SYNC_REF) >= SYNC_OFFSET && awData[0])	ucLevelTick++;
			if(ucLevelTick >= 30) {
				pInfo->nBbpStatus = ERROR_SYNC_NO_SIGNAL+3;
				break;				
			}
		}
		else {
			if(wOperState == 4) {
				if(ucRfLevel <= 0x10) continue;
				
				if(abs(awData[1] - SYNC_REF) < SYNC_OFFSET && awData[1]) continue;
				if(abs(awData[0] - SYNC_REF) < SYNC_OFFSET && awData[0]) continue;
				if(ucRfLevel == 0 || ucIfLevel == 0) continue;

				pInfo->nBbpStatus = ERROR_SYNC_NO_SIGNAL+4;
				break;				
			}
		}
	}
...
INC_UINT8 INC_START(INC_UINT8 ucI2CID, ST_SUBCH_INFO* pChInfo, INC_UINT16 IsEnsembleSame)
{
  ...
  INC_CMD_WRITE(ucI2CID, APB_DEINT_BASE+ 0x1, 0x0000);
  if(IsEnsembleSame) INC_DELAY(25);
...
void INC_SCAN_SETTING(INC_UINT8 ucI2CID)
{
	...
	wTmpValue = INC_CMD_READ(ucI2CID, APB_RF_BASE+ 0x1F)&0xF0;
	wTmpValue |= (T3A00_SCAN_RSSI_AVG&0x0F);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0x1F, wTmpValue);

	INC_CMD_WRITE(ucI2CID, APB_PHY_BASE+ 0x90, 0x0640);	
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0xAD, 0x47);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0xAD, 0x46);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0xAC, 0x47);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0xAC, 0x46);
...
void INC_AIRPLAY_SETTING(INC_UINT8 ucI2CID)
{
	...
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0x22, 0x8C);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0x23, 0x82);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0x31, 0x28);
	INC_CMD_WRITE(ucI2CID, APB_RF_BASE+ 0x32, 0x1E);
	
}

INC_UINT8 INC_CHANNEL_START(INC_UINT8 ucI2CID, ST_SUBCH_INFO* pChInfo)
{
  INC_UINT16 wEnsemble, wOperState;
//  INC_UINT16 wData;
  INC_UINT32 ulRFFreq;
  ST_BBPINFO* pInfo;
#ifdef INC_FICDEC_USE
...
INC_INT16 INC_GET_RSSI(INC_UINT8 ucI2CID)
{
	...
	pInfo = INC_GET_STRINFO(ucI2CID);
	uiRflevel 	= INC_CMD_READ(ucI2CID, APB_VTB_BASE+ 0x06) >> 4 & 0x3f;
	uiAdcValue  = INC_CMD_READ(ucI2CID, APB_RF_BASE+ 0xD1) & 0x3F;
	uiAdcValue = uiAdcValue << 6 | uiRflevel;

	if(uiAdcValue >= aRFAGCTable[0][1]) nRSSI = (INC_INT16) aRFAGCTable[0][0];
