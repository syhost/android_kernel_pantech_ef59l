...
extern void tdmb_spi_recover(unsigned int size);
...
U32 rtvTDMB_GetCER(void)
{
	...
	U32 count = 0, period, cer;
    //U32 OFDM_10=0, FEC_F8=0, FEC_FB=0,Mon_FSM=0;
	...
	//
	{
		U8 ifreg, ifreg2;
		UINT i;
		//static U8 buffer[3 + 188*16];
	
		for (i = 0; i < 10; i++)
		{
			RTV_REG_MAP_SEL(OFDM_PAGE);
			ifreg = RTV_REG_GET(0x00);
			ifreg2 = RTV_REG_GET(0x01);
			if ((ifreg == 0xC6) && (ifreg2 == 0x43))
				break;
			else
			{
				RTV_REG_MAP_SEL(SPI_CTRL_PAGE);
				tdmb_spi_recover(MTV319_SPI_CMD_SIZE + 16*188);
				//tdmb_spi_recover(buffer, MTV319_SPI_CMD_SIZE + 16*188);
	
				RTV_DBGMSG1("[rtvTDMB_GetLockStatus] (%u) Interface error\n", i);
			}
		}
...
#ifdef RTV_CIF_MODE_ENABLED
	if (g_fRtvFicOpened && g_nOpenedSubChNum) {
	#ifdef RTV_REMOVE_NOISE_MSC
		if (cer > 990 && g_fRtvCerMscSet) {
			if (++msc_set_cnt == 3) {
...
S32 rtvTDMB_GetRSSI(void)
{
	...
	case 1:
		nRssi = -(RSSI_RFAGC_VAL(RFAGC, 3.3)
				+ RSSI_GVBB_VAL(GVBB, 0.3) + (20*RTV_TDMB_RSSI_DIVIDER))
				+ 0*RTV_TDMB_RSSI_DIVIDER;
		break;

	case 2:
		nRssi = -(RSSI_RFAGC_VAL(RFAGC, 3.0)
		...
		enum E_RTV_SERVICE_TYPE eServiceType, UINT nThresholdSize)
{
	...
	RTV_DBGMSG0("[rtvTDMB_OpenSubChannel]\n");
	...
	while (1) {
		...
		if (istatus & SPI_INTR_BITS) {
			if (!(istatus & SPI_UNDERFLOW_INTR)) {
				...
				RTV_REG_BURST_GET(0x10, temp_buf,
						g_nRtvInterruptLevelSize);
				ret_size = 384;
				memcpy(pbBuf,temp_buf,ret_size);
...
static inline void print_tsp(uint8 *tspb, UINT size)
{
	...
	{
		RTV_DBGMSG0("[%d] 0x%02X 0x%02X 0x%02X 0x%02X | 0x%02X\n",
...
INT rtvTDMB_ReadTS(U8 *ts_buf)
{
  U8 istatus, ifreg;
  unsigned int intr_size;
  int read_len = 0;

  RTV_GUARD_LOCK;

  //RTV_DBGMSG0("mtv319_read_ts_data  start \n");
  ...
  if (ifreg != 0xAA)
  {
    tdmb_spi_recover(MTV319_SPI_CMD_SIZE + intr_size);
    RTV_DBGMSG0("Interface error 1\n");
  }

  istatus = RTV_REG_GET(0x10);
  if (istatus & (U8)(~SPI_INTR_BITS)) {
    tdmb_spi_recover(MTV319_SPI_CMD_SIZE + intr_size);
    RTV_DBGMSG1("Interface error 2 (0x%02X)\n", istatus);
    goto exit_read_mem;
  }
...
// to debug
 //RTV_DBGMSG1("$$$$$$$$ istatus(0x%02X)\n", istatus);
  ...
  {
    RTV_REG_MAP_SEL(SPI_MEM_PAGE);
    RTV_REG_BURST_GET(0x10, ts_buf, intr_size);

    //print_tsp(ts_buf, intr_size); /* To debug */

    read_len = intr_size;

    if (istatus & SPI_OVERFLOW_INTR)
      RTV_DBGMSG1("OVF: 0x%02X\n", istatus); /* To debug */
  }
  else
  {
    RTV_DBGMSG1("No data interrupt (0x%02X)\n", istatus);
  }
...
{
  RTV_DBGMSG0("[rtvTDMB_CloseFIC] 1\n");
	if (!g_fRtvFicOpened)
		return;
  ...
  RTV_DBGMSG0("[rtvTDMB_CloseFIC] 2\n");
}
...
static void tdmb_InitFEC(void)
{
  RTV_DBGMSG2("[%s] \n", __func__);
...
#ifdef RTV_CIF_MODE_ENABLED
	#if defined(RTV_IF_SPI) || defined(RTV_IF_EBI2)
	RTV_REG_SET(0xEA, 0xE2); /* Header sync byte */
	#endif
#endif
	...
	RTV_REG_SET(0xD3, 0x00);  //FIC Deciding Enable
