...
#if (((defined(T_EF59S) || defined(T_EF59K) || defined(T_EF59L)) && (BOARD_VER > TP10))  || defined(T_EF60S) ||defined(T_EF61K) || defined(T_EF62L)) //p11774 ESD solution
...
#define FPC1080_IOCTL_DISABLE_SENSOR  _IO(FPC1080_IOCTL_MAGIC_NO, 6)
#define FPC1080_IOCTL_ENABLE_SENSOR   _IO(FPC1080_IOCTL_MAGIC_NO, 7)
...
static int fpc1080_power_off_check;
#if (((defined(T_EF59S) || defined(T_EF59K) || defined(T_EF59L)) && (BOARD_VER > TP10))  ||defined(T_EF60S) || defined(T_EF61K) || defined(T_EF62L)) //p11774 ESD solution
...
#if 0
static int fpc1080_suspend(struct spi_device *spi, pm_message_t mesg);

static int fpc1080_resume(struct spi_device *spi);
#endif
...
static int threadfn(void *_fpc1080)
{
	...
	while (!kthread_should_stop()) {
		...
		case FPC1080_THREAD_CAPTURE_MODE:
			...
			DBG_CR( "[DEBUG_FPC1080]  capture mode set.\n" );
			break;

#ifdef CONFIG_FPC1080_NAVIGATION
		case FPC1080_THREAD_NAV_MODE:
			fpc1080_nav_task(fpc1080);
			DBG_CR( "[DEBUG_FPC1080]  navi mode set.\n" );
			break;
#endif
		default:
			break;
			DBG_CR( "[DEBUG_FPC1080]  default at threadfn().\n" );
		}

		if(fpc1080->thread_task.mode != FPC1080_THREAD_EXIT) {
			fpc1080->thread_task.mode = FPC1080_THREAD_IDLE_MODE;
			DBG_CR( "[DEBUG_FPC1080]  idle mode set.\n" );
		}
	}
	return 0;
}
...
static int fpc1080_open(struct inode *inode, struct file *file)
{
	...
	fpc1080_power_off_check = 0;
	gpio_set_value(GPIO_FINGER_POWER, 0);
	usleep(200);
	gpio_set_value(GPIO_FINGER_POWER, 1);
	usleep(200);
	DBG("OPEN_CALLED --> POWER_RESET!\n");
...
static int fpc1080_release(struct inode *inode, struct file *file)
{
	...
	if(fpc1080_mode_change_check == 1)
		...
		status = fpc1080_start_navigation(fpc1080);
			...
			DBG("RELEASE_CALLED --> start_navi!\n");
		}
	else if(fpc1080_power_off_check == 1)
		{
			fpc1080_power_off_check = 0;
			gpio_set_value(GPIO_FINGER_POWER, 0);
			usleep(200);
			...
			usleep(200);
			status = fpc1080_spi_wr_reg(fpc1080, FPC1080_ACTIVATE_DEEP_SLEEP, 0, 1);
			DBG("RELEASE_CALLED --> POWER_RESET!\n");
		}
	else
		{
			status = fpc1080_spi_wr_reg(fpc1080, FPC1080_ACTIVATE_DEEP_SLEEP, 0, 1);
			DBG("RELEASE_CALLED --> Deepsleep!\n");
		}
#endif
	}
	up(&fpc1080->mutex);
...
static int fpc1080_resume(struct spi_device *spi)
{
	...
	/* // p11774 blocked this for mode changing by APP
	if(fpc1080_mode_change == 1)
			{
				init_enhanced_navi_setting_touch(fpc1080); // p11774 touch settings
			}
