...
//1021 patch err static int cyttsp4_suspend(struct device *dev)
int cyttsp4_suspend(struct device *dev)
{
...
int cyttsp4_resume(void *handle)
{
    struct cyttsp4 *ts = handle;
#elif defined(CONFIG_PM_SLEEP)
//1021 patch err static int cyttsp4_resume(struct device *dev)
int cyttsp4_resume(struct device *dev)
{
	struct cyttsp4 *ts = dev_get_drvdata(dev);
#else
int cyttsp4_resume(void *handle)
...
void *cyttsp4_core_init(struct cyttsp4_bus_ops *bus_ops, struct device *dev, int *irq, char *name)
{
    ...
    if (dev == NULL) {
        printk("%s: Error, dev pointer is Null\n", __func__);
        goto error_alloc_data;
    }

    if (bus_ops == NULL) {
        printk("%s: Error, bus_ops Pointer is Null\n", __func__);
        goto error_alloc_data;
    }
    ts = kzalloc(sizeof(*ts), GFP_KERNEL);
    if (ts == NULL) {
        printk("%s: Error, kzalloc context memory\n", __func__);
        goto error_alloc_data;
    }
    ...
    if (ts->cyttsp4_wq == NULL) {
        printk("%s: No memory for cyttsp4_resume_startup_wq\n",__func__);
        goto error_alloc_failed;
    }
    ...
    if (ts->platform_data == NULL) {
        printk("%s: Error, platform data is Null\n", __func__);
        goto error_alloc_failed;
    }

    if (ts->platform_data->frmwrk == NULL) {
        printk("%s: Error, platform data framework is Null\n",__func__);
        goto error_alloc_failed;
    }

    if (ts->platform_data->frmwrk->abs == NULL) {
        printk("%s: Error, platform data framework array is Null\n",__func__);
        goto error_alloc_failed;
    }
    ...
    *irq = gpio_to_irq(GPIO_TOUCH_CHG);
    ts->irq = *irq;
    if (ts->irq <= 0) {
        printk("%s: Error, failed to allocate irq\n", __func__);
        goto error_init;
    }

    /* Create the input device and register it. */
    printk("%s: Create the input device and register it\n", __func__);
    input_device = input_allocate_device();
    if (input_device == NULL) {
        printk("%s: Error, failed to allocate input device\n",__func__);
        goto error_init;
    }
    ...
    printk("%s: Initialize event signals\n", __func__);
    ...
    if (retval < 0) {
        printk("%s: failed to init irq r=%d name=%s\n",__func__, retval, ts->input->name);
    ...
    if (retval < 0) {
        printk("%s: Error, failed to register input device r=%d\n",__func__, retval);
        goto error_init;
    } 

	init_hw_setting(&input_device->dev);
...
error_alloc_data:
	printk("%s: Failed Initialization\n", __func__);
no_error:

	printk("%s: Initialization OK!!\n", __func__);
