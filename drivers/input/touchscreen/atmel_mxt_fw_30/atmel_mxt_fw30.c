...
#ifdef PAN_TOUCH_GOLDREFERENCE_CHECK
#include "../../../../include/pantech/sky_rawdata.h" // jinwook.lee_20140123
#include <linux/fs.h>
#include <linux/file.h>
#include <asm/uaccess.h>
#endif

//#include "../../../../../vendor/pantech/include/BOARD_REV.h" // jinwook.lee_20140123
...
#ifdef PAN_TOUCH_GOLDREFERENCE_CHECK
static int read_offset_from_rawdata_for_goldreference(unsigned int offset, unsigned int read_size, char* read_buf);
static void read_goldreference_funcflag(void);
static int write_offset_from_rawdata_for_goldreference(unsigned int offset, unsigned int read_size, char* read_buf);

#endif

#ifdef PAN_T15_KEYARRAY_ENABLE
//p13106 EF59 HOMEKEY disable function. 13.08.23
void set_smart_cover_key( int cover);
#endif
...
#define PAN_GLD_REF_TIMEOUT_CHECK           7000	// 5sec
#define PAN_GOLD_EXIST_CHECK_INTERVAL       30000	// 60sec -> 30sec  p11309 - 2013.07.26

#define PAN_GLD_REF_TIMEOUT_CNT             5		// 3 - p11309 - 2013.07.29

#if defined(CONFIG_MACH_MSM8974_EF60S) || defined(CONFIG_MACH_MSM8974_EF61K) || defined(CONFIG_MACH_MSM8974_EF62L) || defined(CONFIG_MACH_MSM8974_EF63S) || defined(CONFIG_MACH_MSM8974_EF63K) || defined(CONFIG_MACH_MSM8974_EF63L)
#define PAN_GLD_REF_DELTA_LIMITATION        60
#else 
#define PAN_GLD_REF_DELTA_LIMITATION        50		// obj_multi_touch_t100[1].intthr * 4  // Glove mode intthr value - 2013.07.29
#endif
...
static struct timer_list pan_gold_exist_timer;
static struct work_struct pan_gold_exist_timer_work_queue;
struct workqueue_struct *pan_gold_exist_timer_wq;
static void gold_exist_timer_func(unsigned long data);
void gold_exist_timer_wq_func(struct work_struct * p);

static int isGoldref_on = PAN_GOLDREFERENCE_BOOT_INIT;
static unsigned int isBad_data = 0;
static int isGoldcheck_cnt = 0;
#endif
...
#ifdef PAN_T15_KEYARRAY_ENABLE	
	//p13106 EF59 HOMEKEY disable function. 13.08.23
	set_smart_cover_key(mTouch_cover_status_hallic);	
#endif
...
//-- p11309
		break;
	case TOUCH_IOCTL_GLD_REF_GET_GOLDREFERENCE_ON_STATE:
#ifdef PAN_TOUCH_GOLDREFERENCE_CHECK
		if (copy_to_user(argp, &isGoldref_on, sizeof(isGoldref_on))) {
			dbg_ioctl(" TOUCH_IOCTL_GLD_REF_GET_GOLDREFERENCE_ON_STATE is %d\n",isGoldref_on);
		}
#endif
    break;
...
#ifdef PAN_TOUCH_GOLDREFERENCE_CHECK
			isGoldref_on = PAN_GOLDREFERENCE_FLAG_ON_AND_GR_ON;
...
//++ p11309 - 2013.09.06 for Touch Pen Insertion Check
	...
	case TOUCH_IOCTL_CHECK_TOUCH_PEN:
		dbg_op("[%s] Touch Pen Status: %d\n", __func__, pen_switch->sdev.state);
		if (copy_to_user(argp, &pen_switch->sdev.state, sizeof(pen_switch->sdev.state))) {
...
#endif
#ifdef PAN_TOUCH_GOLDREFERENCE_CHECK
		if(strncmp(buf, "isgoldref",9)==0)
		{			
			dbg_cr("isgoldreference is %d\n",isGoldref_on);
		}

		if(strncmp(buf, "baddata_on",10)==0){     
			isGoldref_on |= 2;
			isBad_data++;
			dbg_cr(" Gold Referecne status: Bad Stored Data. cnt -> %d\n",isBad_data);
		}	

		if(strncmp(buf, "read_func_flag",14)==0){
			read_goldreference_funcflag();
		}
#endif
		if(strncmp(buf, "read_hallic",11)==0){
			dbg_cr("hall-ic state -> %d\n",gpio_get_value(9));
		}
...
//++ p11309 - 2013.09.24 for prevent to enter sleep mode during getting gold reference.
	if ( mxt_fw30_data->state == SUSMODE )
	{
...
//-- p11309
		...
		dbg_cr("pan_gld_done cnt is wrong \n");
	}
}
//-- p11309
...
#ifdef PAN_TOUCH_GOLDREFERENCE_CHECK
	if(isGoldref_on != PAN_GOLDREFERENCE_FLAG_ON_AND_GR_ON ){
  	acquisition_config.tchautocal   = 10;
  	acquisition_config.atchcalst    = 0;
  	acquisition_config.atchcalsthr  = 0;	
	goldenreferences_t66_config.ctrl = 0;	
...
static void gold_exist_timer_func(unsigned long data)
{
	dbg_cr("Start gold_exist_timer_func...\n");
	queue_work(pan_gold_exist_timer_wq, &pan_gold_exist_timer_work_queue);
	return;
}
...
static void read_goldreference_funcflag(void)
{
  ...
  dbg_cr("read_goldreference_funcflag\n");
  rc = read_offset_from_rawdata_for_goldreference(FUNCTEST_RESULT_INIT_START, SECTOR_SIZE, read_buf);
  if(rc){
    dbg_cr("read_offset_from_rawdata_for_goldreference is failed -> %d, cnt -> %d\n",rc,isGoldcheck_cnt);
    if(isGoldcheck_cnt < 5){
      mod_timer(&pan_gold_exist_timer, jiffies + msecs_to_jiffies(PAN_GOLD_EXIST_CHECK_INTERVAL));
      isGoldcheck_cnt++;
    }
  }
  else 
  {
    ...
    if ((int)read_buf[204] == 1){
      isGoldref_on |= 0x1;
      mxt_Acquisition_Config_T8_Init();
      dbg_cr("Touch Gold Reference Existence check is success. isGoldref_on value is %d\n",isGoldref_on);
      if(isGoldref_on == 3){
        read_buf[204] = 2;
        rc = write_offset_from_rawdata_for_goldreference(FUNCTEST_RESULT_INIT_START, SECTOR_SIZE, read_buf);
        if( rc ){
          dbg_cr("Touch Gold Reference fucntion flga write zero faild. isGoldref_on value is %d\n",isGoldref_on);
        }else{
          dbg_cr("Touch Gold Reference fucntion flga write zero success. isGoldref_on value is %d\n",isGoldref_on);
        }
      }
    }else{
      dbg_cr("read_offset_from_rawdata_for_goldreference is success but there is no goldreference.\n");
    }
  }
  return;
}
...
static int read_offset_from_rawdata_for_goldreference(unsigned int offset, unsigned int read_size, char* read_buf)
{
	...
	{
		set_fs(oldfs);
		dbg_cr("%s: filp_open error\n",__func__);
		return 1;
	}
	set_fs(oldfs);
	dbg_cr("%s: file open OK\n", __func__);
		
	rawdata_filp->f_pos = offset;
	if (((rawdata_filp->f_flags & O_ACCMODE) & O_RDONLY) != 0)
	{
		dbg_cr("%s: rawdata read - permission denied!\n", __func__);
	...
	{
		set_fs(oldfs);
		dbg_cr("%s: read fail from rawdata = %d \n",__func__,rc);
		filp_close(rawdata_filp, NULL);
		return 3;
	}
	set_fs(oldfs);
...
static int write_offset_from_rawdata_for_goldreference(unsigned int offset, unsigned int read_size, char* read_buf)
{
	...
	oldfs = get_fs();
	set_fs(KERNEL_DS);
	rawdata_filp = filp_open("/dev/block/mmcblk0p11", O_WRONLY | O_SYNC, 0);
	if( IS_ERR(rawdata_filp) )
	{
		set_fs(oldfs);
		dbg_cr("%s: filp_open error\n",__func__);
	...
	rawdata_filp->f_pos = offset;
	if (((rawdata_filp->f_flags & O_ACCMODE) & O_WRONLY) == 0)
	{
		dbg_cr("%s: rawdata write - permission denied!\n f_flag -> %x", __func__,rawdata_filp->f_flags);
		filp_close(rawdata_filp, NULL);
		return 2;
	}
	...
	oldfs = get_fs();
	set_fs(KERNEL_DS);
	rc = rawdata_filp->f_op->write(rawdata_filp, read_buf, read_size, &rawdata_filp->f_pos);
	if (rc < 0) 
	{
		set_fs(oldfs);
		dbg_cr("%s: write fail from rawdata = %d \n",__func__,rc);
		filp_close(rawdata_filp, NULL);
		return 3;
	}
	set_fs(oldfs);
...
//++ p11309 - 2013.08.05 for Improvement get gold reference
		...
		//	calibrate_chip();
          ...
          //++ p11309 - 2013.08.05 for check Third Delta
				  // if delta is success
...
{ 
  ...
  state = !gpio_get_value(TOUCH_PEN_GPIO);
	...
	wake_lock_timeout(&touch_pen_wakelock,msecs_to_jiffies(TOUCH_PEN_WAKELOCK_TIMEOUT));
...
//++ p11309 - 2013.05.14 for gold reference T66 // 2013.09.24 revised
...
#if defined (OFFLINE_CHARGER_TOUCH_DISABEL) || defined (PAN_TOUCH_GOLDREFERENCE_CHECK)
...
//-- p11309 
  ...
  pen_switch->sdev.state=-1;    // state init.
  #if (BOARD_VER < WS20)
  rc = gpio_get_value(TOUCH_PEN_GPIO);
  #else
  rc = !gpio_get_value(TOUCH_PEN_GPIO);
  #endif
  //printk("[TOUCH_PEN] LEVEL -> %d\n",rc);
  ...
  device_init_wakeup(pen_switch->sdev.dev, 1);
  enable_irq_wake(pen_switch->irq);
...
#ifdef PAN_TOUCH_GOLDREFERENCE_CHECK
	init_timer(&pan_gold_exist_timer);
	pan_gold_exist_timer.function = gold_exist_timer_func;
	pan_gold_exist_timer.data = 0;
	pan_gold_exist_timer_wq = create_singlethread_workqueue("pan_gold_exist_timer_wq");
	if (!pan_gold_exist_timer_wq){
		dbg_cr("create_singlethread_workqueue(pan_gold_exist_timer_wq) error.\n");
		return -ENOMEM;
	}
	INIT_WORK(&pan_gold_exist_timer_work_queue, gold_exist_timer_wq_func);
	mod_timer(&pan_gold_exist_timer, jiffies + msecs_to_jiffies(PAN_GOLD_EXIST_CHECK_INTERVAL));
#endif	
//-- p11309
...
void pan_pmode_autocal_wq_func(struct work_struct * p) 
{
	...
	dbg_touch("[PMODE] Autocal is disabled\n");
...
void pan_pmode_antical_wq_func(struct work_struct * p) 
{
	...
	dbg_touch("[PMODE] Antical is disabled\n");
...
void __exit mxt_exit(void)
{
  ...
  remove_proc();
	...
	dbg_func_out();
#ifdef PAN_TOUCH_PEN_DETECT	
  device_init_wakeup(pen_switch->sdev.dev, 0);
  switch_dev_unregister(&pen_switch->sdev);	  
