...
//============================================================
#define GPIO_TOUCH_CHG_INT		60
#define IRQ_TOUCH_INT			gpio_to_irq(GPIO_TOUCH_CHG_INT)

//	touch ic digital power for i2c 
#define TOUCH_POWER_DVDD		"8941_lvs1"  // p11223
...
#define SKY_PROCESS_CMD_KEY 0//TOUCH_REACTION_RATE // p11774 blocked for test
#define TOUCH_IO 0 // p11774 blocked for test
//============================================================
#define MAX_CH 364
#ifdef TOUCH_IO
...
static void melfas_ts_get_data(struct work_struct *work)
{
	...
	if (buf[0] == 0x0F) // chip reset
	{
		...
		init_hw_setting();
#if (SKY_ANDROID_ICS && SKY_MULTI_TOUCH_PROTOCOL_B)
		input_mt_init_slots(ts->input_dev, MELFAS_MAX_TOUCH);  // p11774_20130222 
#endif 
		...
		for(i=0; i<read_num; i=i+6)
		{
			...
			if ( g_Mtouch_info[FingerID].strength <= 0 && strength_temp > 0 ) {
				g_track_id[FingerID] = input_mt_new_trkid(ts->input_dev);  // p11774_20130222
...
//	}
	...
	ts->input_dev->name = "melfas_ts_input"; //.kl file // p11223
...
//	ts->input_dev->evbit[0] =  BIT_MASK(EV_SYN) | BIT_MASK(EV_ABS) | BIT_MASK(EV_KEY);	
	set_bit(INPUT_PROP_DIRECT, ts->input_dev->propbit);		// JB Update // p11223

#ifdef	SKY_PROCESS_CMD_KEY
//	set_bit(KEY_RECENTAPPS, ts->input_dev->keybit);  // p11774_Touch_key // p11223  // blocked for test
