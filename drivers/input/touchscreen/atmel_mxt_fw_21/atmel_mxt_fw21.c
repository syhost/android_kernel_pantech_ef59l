...
#include "../../../../../vendor/pantech/include/BOARD_REV.h" // mirinae_20110920
#include "atmel_mxt_fw21.h"
    ...
    #include "ef59s/atmel_540s_21_cfg_multi.h"
  #else
	#include "ef60s/atmel_540s_21_cfg_multi.h"
  #endif
...
static uint8_t cal_check_flag = 0u;                                   /* flag to indicate if last calibration has been confirmed good */
	...
	case TOUCH_IOCTL_MULTI_TSP_OBJECT_SEL:
		...
		if ( arg < TOUCH_MODE_MAX_NUM ) {
		  if(mTouch_mode == (int)arg){
				dbg_op("[touch_fops] Touch Mode selection is equal, mTouch_mode -> %d\n", mTouch_mode);
			}
			else
			{
			mTouch_mode = arg;
...
//-- p11309 
  ...
  case TOUCH_IOCTL_DO_KEY:
	...
	case TOUCH_IOCTL_MULTI_TSP_OBJECT_SEL:
		...
		if ( arg < TOUCH_MODE_MAX_NUM ) {
		  if(mTouch_mode == (int)arg){
				dbg_op("[touch_monitor] Touch Mode selection is equal, mTouch_mode -> %d\n", mTouch_mode);
			}
			else
			{
			mTouch_mode = arg;
...
static void gold_exist_timer_func(unsigned long data)
{
	dbg_op("Start gold_exist_timer_func..\n");
...
static void read_goldreference_funcflag(void)
{
  ...
  dbg_op("read_goldreference_funcflag\n");
  ...
  else 
  {
    if ((int)read_buf[204] == 1){
      dbg_op("Touch Gold Reference Existence check is success\n");
	  isGoldref_on = 1;
      mxt_Acquisition_Config_T8_Init();
...
//       if(isGoldcheck_cnt < 5){
//         dbg_cr("read_offset_from_rawdata_for_goldreference is success but there is no goldreference. cnt -> %d\n",isGoldcheck_cnt);
//         mod_timer(&pan_gold_exist_timer, jiffies + msecs_to_jiffies(PAN_GOLD_EXIST_CHECK_INTERVAL));
//         isGoldcheck_cnt++;
//       }
    }
...
#if defined(PAN_MULTI_OBJECT_ENABLE)
#else
			acquisition_config.atchcalst = obj_acquisition_config_t8[mTouch_mode][T8_ATCHCALST]/*2*/;
			acquisition_config.atchcalsthr = obj_acquisition_config_t8[mTouch_mode][T8_ATCHCALSTHR]/*50*/;
			acquisition_config.atchfrccalthr = obj_acquisition_config_t8[mTouch_mode][T8_ATCHFRCCALTHR];     /*!< Anti-touch force calibration threshold */
			acquisition_config.atchfrccalratio = obj_acquisition_config_t8[mTouch_mode][T8_ATCHFRCCALRATIO];  /*!< Anti-touch force calibration ratio */  
#endif
			dbg_op(" write acq atchcalst=%d, atchcalsthr=%d\n", acquisition_config.atchcalst, acquisition_config.atchcalsthr );
			if (write_acquisition_T8_config(acquisition_config) != CFG_WRITE_OK)
			{
				/* "Acquisition config write failed!\n" */
				dbg_cr("\n[TSP][ERROR] line : %d\n", __LINE__);
...
int8_t get_touch_antitouch_info(void) {
	...
	if((data_buffer[0] == 0xF3) && (data_buffer[1] == 0x00))
	{
		...
		x_line_limit = 16 + cte_t46_config.reserved;
...
U8 write_mem(U16 start, U8 size, U8 *mem)
{
	...
	if(ret < 0) {
		do{
			...
			msleep(20);
		}while(mxt_fw21_i2c_write(start,mem,size)<0);
		if(ret<0){
