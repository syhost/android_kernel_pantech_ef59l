/* kernel/driver/misc/felica/felica_master.c
 *
...
static struct of_device_id felica_match_table[] = {
	{ .compatible = "sony, felica", },
	{}
};
...
static int felica_probe(struct platform_device *pdev)
{
	...
	/* Load platform data of FeliCa driver */
	if (pdev->dev.of_node) {
		pr_info(PRT_NAME ":%s of_node\n", __func__);
		pfdata = kzalloc(sizeof(struct felica_platform_data), GFP_KERNEL);

		if(!pfdata) {
			pr_err(PRT_NAME"  : unable to allocate platform data\n"); 
			ret = -ENOMEM;
			goto err_get_platform_data;
		}
		pfdata->gpio_pon = (unsigned int)of_get_named_gpio_flags(pdev->dev.of_node, "sony,gpio_pon", 0, NULL);
		pfdata->gpio_rfs = (unsigned int)of_get_named_gpio_flags(pdev->dev.of_node, "sony,gpio_rfs", 0, NULL);
		pfdata->gpio_int = (unsigned int)of_get_named_gpio_flags(pdev->dev.of_node, "sony,gpio_int", 0, NULL);
		pfdata->gpio_intu = (unsigned int)of_get_named_gpio_flags(pdev->dev.of_node, "sony,gpio_intu", 0, NULL);
		pfdata->gpio_temp= (unsigned int)of_get_named_gpio_flags(pdev->dev.of_node, "sony,gpio_temp", 0, NULL);
		pfdata->gpio_hsel = (unsigned int)of_get_named_gpio_flags(pdev->dev.of_node, "sony,gpio_hsel", 0, NULL);
		pfdata->gpio_flt  = (unsigned int)of_get_named_gpio_flags(pdev->dev.of_node, "sony,gpio_flt", 0, NULL);
	...
	ret = snfc_flt_probe_func(felica);
	if (ret) {
