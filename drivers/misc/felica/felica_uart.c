...
#define UART_PORT_NAME "/dev/ttyHSL2"

#define FELICA_UART_MAX_TIME   200
#define FELICA_OPEN_TIMEOUT    5000

#define FELICA_RDONLY       1
#define FELICA_WRONLY       2
#define FELICA_RW           4

static void * g_tty_data = NULL;
static unsigned int g_file_flag =0;
...
static int oem_uart_open(struct inode *inode, struct file *filp)
{
	...
	/* open tty driver */
	tty = filp_open(UART_PORT_NAME, O_RDWR | O_NOCTTY, 0);
	if (IS_ERR(tty)) {
...
static int felica_uart_open(struct inode *inode, struct file *file)
{
	...
	pr_debug(PRT_NAME ": %s: 0x%x, prvate_data =0x%p\n", __func__,file->f_flags, file->private_data);
	...
	if ((file->f_flags & O_ACCMODE) == O_RDONLY) {
		if ((g_file_flag & FELICA_RDONLY) == 0)  {   
			g_file_flag |= FELICA_RDONLY;
			inc_flag =1;
		}
	}else if ((file->f_flags & O_ACCMODE) == O_WRONLY) {
		if ((g_file_flag & FELICA_WRONLY) == 0)  {   
			g_file_flag |= FELICA_WRONLY;
			inc_flag =1;
		}
		...
		pr_err(PRT_NAME":[%s] file flag error : f_flags =0x%x\n",__func__,file->f_flags); 
	}
	...
	if (atomic_read(&felica_drv->felica_uart_cnt) == 1) {
		...
		g_tty_data = file->private_data;
	}

	if (g_tty_data != NULL) {
		file->private_data = g_tty_data;
	}
...
static int felica_uart_release(struct inode *inode, struct file *file)
{
	...
	if (g_tty_data != NULL) {
		if (g_tty_data != file->private_data) {
			pr_err(PRT_NAME ": %s:private_data = 0x%p, g_tty_data = 0x%p\n", __func__,file->private_data, g_tty_data);
			return 1;
		}
	} else {
		pr_err(PRT_NAME ": %s: g_tty_data is  NULL: private_data = 0x%p\n", __func__,file->private_data);
		return 1;
	}
					...
					size_t count, loff_t *offset)
{
	...
	if (len > 0) {
		tty = (struct file *)file->private_data;
		if (tty) {
