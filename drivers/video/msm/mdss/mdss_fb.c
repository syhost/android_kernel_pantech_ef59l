...
#include "mdss_dsi_sharp_panel_ctl.h"
#endif
...
int poweroff_shutdown_flag = false;
#endif
				      ...
				      enum led_brightness value)
{
	...
	printk("Framework BL : %d \n",bl_lvl);

	if (!bl_lvl && value)
		bl_lvl = 1;
#ifdef CONFIG_F_SKYDISP_BACKLIGHT_DEBUG
	led_cdev->kernel_brightness = bl_lvl;
#endif
...
#ifdef CONFIG_F_SKYDISP_FACTORY_SLEEP_ENABLE_I
char panel_state;
...
/*
#ifdef CONFIG_MACH_MSM8974_NAMI
	static int nami_lcd_first_boot = 0;
#endif
*/
...
/*
#ifdef CONFIG_MACH_MSM8974_NAMI
	if(!nami_lcd_first_boot){
		rc = load_565rle_image(INIT_IMAGE_FILE,0);
		if(!rc ){
			rc = mdss_fb_blank_sub(FB_BLANK_UNBLANK, mfd->fbi,1);
		}
		mdss_fb_pan_display(&mfd->fbi->var,mfd->fbi);
	}
	nami_lcd_first_boot= true;
	 mdss_dsi_panel_sec_on(pdata);
#endif	
*/
...
#ifdef CONFIG_MACH_MSM8974_NAMI
	rc = sharp_panel_sysfs_register(mfd->fbi->dev);
	if (rc < 0)
		printk(KERN_ERR "Unable to create sysfs interface.\n");
#endif
	return rc;
}
...
static void  mdss_fb_shutdown(struct platform_device *pdev)
{
	...
	struct mdss_panel_info *pdata = mfd->panel_info;
	struct mdss_panel_data * panel_pdata =NULL;
	
	panel_pdata = container_of(pdata, struct mdss_panel_data,
				panel_info);

	panel_pdata->silent_flag = true;
#endif
	if (!mfd)
		return ;
	...
	ret = mdss_fb_send_panel_event(mfd, MDSS_EVENT_BLANK, NULL);
	if (ret) {
		pr_err("unable to suspend fb%d (%d)\n", mfd->index, ret);
	}
	mdss_fb_pan_idle(mfd);
	
	poweroff_shutdown_flag = true;
	
	ret = mdss_fb_send_panel_event(mfd, MDSS_EVENT_PANEL_OFF, NULL); 
	if (ret) {
		pr_err("mdss_fb_send_panel_event err\n");
	}
	/*
	ret = mdss_dsi_panel_power_on(panel_pdata, 0);
	if (ret) {
...
void mdss_fb_set_backlight(struct msm_fb_data_type *mfd, u32 bkl_lvl)
{
	...
	if ((!mfd->panel_power_on || !bl_updated) && !IS_CALIB_MODE_BL(mfd)) {
		...
		return;
		 ...
		 if(mfd->panel_power_on && bl_updated) /* shinbrad Fixed for encryption backlight bug 131010 */ 
			unset_bl_level = bkl_lvl;
	}
...
#if defined(D_SKY_BACKLIGHT_SCHEDULE_DEBUG)
static int pre_unset_bl_level;
static int pre_bl_updated;
#endif
void mdss_fb_update_backlight(struct msm_fb_data_type *mfd)
{
	struct mdss_panel_data *pdata;
#if defined(D_SKY_BACKLIGHT_SCHEDULE_DEBUG)
	if((unset_bl_level != pre_unset_bl_level) || ( bl_updated != pre_bl_updated))
	{
		printk("[%s] Enter...unset_bl_level= %d , bl_updated= %d\n",__func__, unset_bl_level, bl_updated);
		pre_unset_bl_level = unset_bl_level;
		pre_bl_updated = bl_updated;
	}
#endif
//20130706 ejkim77 panel_power_on condition add
//	if (unset_bl_level && !bl_updated && mfd->panel_power_on) {
//20130927 shinbrad panel_power_on condition rollback
...
static int mdss_fb_open(struct fb_info *info, int user)
{
	...
	}else if(mfd->ref_cnt == 3 && chk_flag == 1){
		mfd->ref_cnt--;
		chk_flag ++;
