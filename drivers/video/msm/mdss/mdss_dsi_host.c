...
//#define CONFIG_F_SKYDISP_BUGON_TEST
#ifdef CONFIG_F_SKYDISP_BUGON_TEST
int bug_check = 0;
#endif
#ifdef CONFIG_F_SKYDISP_SHUTDOWN_BUGFIX
extern int poweroff_shutdown_flag;
#endif
			     ...
			     struct mdss_panel_data *pdata)
{
	...
	if (mode == DSI_VIDEO_MODE) {
       ...
       if(mode == DSI_VIDEO_MODE)
        {
		data = MIPI_INP((ctrl_pdata->ctrl_base) + 0x0010);
                data |= BIT(12); // BLLP_POWER_STOP
		MIPI_OUTP((ctrl_pdata->ctrl_base) + 0x0010, data);
        }
#if 0  //DSI_COMMAND_MODE LCD OFF command tx interrupt dosen't  occurs.	   
	   else
        {
		data = MIPI_INP((ctrl_pdata->ctrl_base) + 0x0010);
                data &= ~BIT(12); // BLLP_POWER_STOP
...
void dumpreg(unsigned char *ctrl_base) 
{ 
u32 tmp0x0,tmp0x4,tmp0x8,tmp0xc; 
int i; 
pr_err("%s: =============DSI Reg DUMP==============\n", __func__); 
for(i=0; i< 91; i++){ 
tmp0x0 = MIPI_INP(ctrl_base+(i*16)+0x0); 
tmp0x4 = MIPI_INP(ctrl_base+(i*16)+0x4); 
tmp0x8 = MIPI_INP(ctrl_base+(i*16)+0x8); 
tmp0xc = MIPI_INP(ctrl_base+(i*16)+0xc); 
...
#if 0  /* shinjg ... Codes for dma timeout debuging */
		dumpreg(ctrl->ctrl_base); 
		WARN(1, "DMA timeout!\n");
#endif 
	}
...
{
	unsigned long flag;
#if defined(CONFIG_QUALCOMM_BUGFIX_INTR_VIDEO_DONE_FOR_VIDEO_MODE_PANEL) /*shin test*/
	u32 data;
