...
int mdss_mdp_smp_reserve(struct mdss_mdp_pipe *pipe)
{
	...
	int rc = 0, rot_mode = 0;
	u32 nlines;
#ifdef CONFIG_F_SKYDISP_FIXGPU_FALLBACK_ERROR
	u16 width;
...
#ifdef CONFIG_QUALCOMM_BUGFIX_BWC_SMP
 		if (pipe->mixer && pipe->mixer->rotator_mode) {
			rot_mode = 1;
			} else if (ps.num_planes == 1) {
			u32 w = max(pipe->mixer->width, pipe->src.w);
				ps.ystride[0] = w * MAX_BPP;
			}
#else
   		if (pipe->mixer && pipe->mixer->rotator_mode)
			rot_mode = 1;
...
#endif
#endif
#ifdef CONFIG_QUALCOMM_BUGFIX_UHD_UNDERRUN
...
#ifdef CONFIG_QUALCOMM_BUGFIX_BWC_SMP
		/*
		 * For bwc, stride already contains the size that is going to be
		 * stored in SMP which includes 2 or 4 lines.
		 * For other cases, it needs a minimum of two lines: one to
		 * fetch data from bus and other to process down the pipeline.
		 */
#endif
...
#ifdef CONFIG_QUALCOMM_BUGFIX_BWC_SMP
 				fetch_size = ps.ystride[i] * nlines;
				num_blks = DIV_ROUND_UP(fetch_size, SMP_MB_SIZE);
