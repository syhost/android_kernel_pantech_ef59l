...
static int mdss_dsi_regulator_init(struct platform_device *pdev)
{
	...
	if (ctrl_pdata->power_data.num_vreg > 0) {
		...
		ret = regulator_set_voltage(dsi_drv->vdd_io_vreg, 2200000,
				2200000);
	if (ret) {
	...
	if (enable) {
...
#if (BOARD_VER >= WS20)
			gpio_set_value((ctrl_pdata->lcd_vddio_switch_chip_en), 1);
                     msleep(50);
		...
		if (ctrl_pdata->power_data.num_vreg > 0) {
			...
			gpio_set_value((ctrl_pdata->lcd_vddio_reg_en), 0);
                     msleep(50);
			gpio_set_value((ctrl_pdata->lcd_vddio_switch_chip_en), 0);
		...
		if (pdata->panel_info.panel_power_on == 0)
			...
			if (ctrl_pdata->power_data.num_vreg > 0) {
				...
				ret = regulator_set_optimum_mode(
					(ctrl_pdata->shared_pdata).vdd_io_vreg, 100);
				if (ret < 0) {
					pr_err("%s: vdda_vreg io set opt mode failed.\n",
						__func__);
					return ret;
				}
			}
...
static int mdss_dsi_blank(struct mdss_panel_data *pdata)
{
	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
#ifdef CONFIG_F_SKYDISP_SILENT_BOOT	//seunghwa_Ji p13832
	static bool first_silent = false;
#endif	
	pr_debug("%s+:\n", __func__);
...
#ifdef CONFIG_F_SKYDISP_SILENT_BOOT	//seunghwa_Ji p13832
	if(pdata->silent_flag != true){
		if(first_silent == true)
...
#ifdef CONFIG_F_SKYDISP_SILENT_BOOT	//p12281 	
		if(pdata->silent_backlight == true) return 0; 
#endif			
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_PANEL_CLK_CTRL:
		if(pdata->panel_info.panel_power_on == 0 && pdata->panel_info.type == MIPI_CMD_PANEL) 
			pr_err("%s: skip MDSS_EVENT_PANEL_CLK_CTRL\n", __func__);
		else
			      ...
			      struct mdss_panel_common_pdata *panel_data)
{
	...
	printk("[shinbrad:%s] mipi->dsi_pclk_rate = dsi_pclk_rate : %d\n",__func__, dsi_pclk_rate); //shinbrad
	...
	ctrl_pdata->lcd_vcip_reg_en = of_get_named_gpio(pdev->dev.of_node,
						     "qcom,enable-reg-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->lcd_vcip_reg_en)) {
		pr_err("%s:%d, Disp_en gpio not specified\n",
						__func__, __LINE__);
	} else {
		rc = gpio_request(ctrl_pdata->lcd_vcip_reg_en, "vci_reg_en");
		if (rc) {
			pr_err("request reset gpio failed, rc=%d\n",
			       rc);
			gpio_free(ctrl_pdata->lcd_vcip_reg_en);
			return -ENODEV;
		}
	}
...
#if (BOARD_VER >= WS20)
	ctrl_pdata->lcd_vddio_switch_chip_en = of_get_named_gpio(pdev->dev.of_node,
						 "qcom,vddio-switch-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->lcd_vddio_switch_chip_en)) {
		pr_err("%s:%d, 1.8v Ext Regulator switch gpio not specified\n",
						__func__, __LINE__);
	} else {
		rc = gpio_request(ctrl_pdata->lcd_vddio_switch_chip_en, "lcd_vddio_ext_reg_switch");
		if (rc) {
			pr_err("request 1.8v Ext Regulator switch gpio failed, rc=%d\n",
				rc);
			gpio_free(ctrl_pdata->lcd_vddio_switch_chip_en);
			if (gpio_is_valid(ctrl_pdata->bl_en_gpio))
				gpio_free(ctrl_pdata->bl_en_gpio);
			return -ENODEV;
		}
		rc = gpio_direction_output(ctrl_pdata->lcd_vddio_switch_chip_en, 1);
		if (rc) {
...
/* +5.6V regulator */		
	ctrl_pdata->lcd_vcip_reg_en = of_get_named_gpio(pdev->dev.of_node,
						 "qcom,vcip-reg-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->lcd_vcip_reg_en)) {
		pr_err("%s:%d, +5.6v Ext Regulator gpio not specified\n",
						__func__, __LINE__);
	} else {
		rc = gpio_request(ctrl_pdata->lcd_vcip_reg_en, "lcd_vcip_ext_reg");
		if (rc) {
		...
		rc = gpio_direction_output(ctrl_pdata->lcd_vcip_reg_en, 1);
		if (rc) {
...
/* -5.4V regulator */		
	ctrl_pdata->lcd_vcin_reg_en = of_get_named_gpio(pdev->dev.of_node,
						 "qcom,vcin-reg-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->lcd_vcin_reg_en)) {
		pr_err("%s:%d, -5.4v Ext Regulator gpio not specified\n",
						__func__, __LINE__);
	} else {
		rc = gpio_request(ctrl_pdata->lcd_vcin_reg_en, "lcd_vcin_ext_reg");
		if (rc) {
			pr_err("request -5.4v Ext Regulator gpio failed, rc=%d\n",
				rc);
			gpio_free(ctrl_pdata->lcd_vcin_reg_en);
			if (gpio_is_valid(ctrl_pdata->bl_en_gpio))
				gpio_free(ctrl_pdata->bl_en_gpio);	
#if (BOARD_VER >= WS20)					
			if (gpio_is_valid(ctrl_pdata->lcd_vddio_switch_chip_en))
		...
		rc = gpio_direction_output(ctrl_pdata->lcd_vcin_reg_en, 1);
		if (rc) {
			pr_err("set_direction for lcd_vci_ext_reg gpio failed, rc=%d\n",
			       rc);
			gpio_free(ctrl_pdata->lcd_vcin_reg_en);
	...
	if (!gpio_is_valid(ctrl_pdata->bl_swire_gpio)) {
		...
		rc = gpio_request(ctrl_pdata->bl_swire_gpio, "lcd_vddio_ext_reg");
		if (rc) {
			pr_err("request 1.8v Ext Regulator gpio failed, rc=%d\n",
				rc);
			gpio_free(ctrl_pdata->bl_swire_gpio);
		...
		rc = gpio_direction_output(ctrl_pdata->bl_swire_gpio, 1);
		if (rc) {
			pr_err("set_direction for lcd_vddio_ext_reg gpio failed, rc=%d\n",
			       rc);
			gpio_free(ctrl_pdata->bl_swire_gpio);
...
/* -5.4V regulator */		
	...
	ctrl_pdata->cabc_cmds = panel_data->cabc_cmds;
#endif	
	...
	if (rc) {
		...
		if (ctrl_pdata->rst_gpio)
			gpio_free(ctrl_pdata->rst_gpio);
		if (ctrl_pdata->lcd_vcip_reg_en)
			gpio_free(ctrl_pdata->lcd_vcip_reg_en);
#if defined (CONFIG_MACH_MSM8974_EF56S) ||defined(CONFIG_MACH_MSM8974_EF57K) || defined (CONFIG_MACH_MSM8974_EF58L) 	
		if (ctrl_pdata->lcd_vcin_reg_en)
			gpio_free(ctrl_pdata->lcd_vcin_reg_en);		
#endif
		if (gpio_is_valid(ctrl_pdata->disp_en_gpio))
			gpio_free(ctrl_pdata->disp_en_gpio);
		if(ctrl_pdata->bl_en_gpio)
			gpio_free(ctrl_pdata->bl_en_gpio);
#if (BOARD_VER >= WS20)		
		if(ctrl_pdata->bl_swire_gpio)
			gpio_free(ctrl_pdata->bl_swire_gpio);		
#endif		
		return rc;
	}
