...
#define CONFIG_D_SKY_MIPI_CLK_HIGH
#endif
...
struct mdss_panel_common_pdata {
	...
	struct dsi_panel_cmds on_sec_cmds;
#endif
};
...
struct dsi_drv_cm_data {
	...
	struct regulator *vdd_io_rsp_vreg;	
...
struct mdss_dsi_ctrl_pdata {
	...
	int pwm_lpg_chan;
	int bklt_max;
	int lcd_vcip_reg_en;       //shinjg p13447 samsung oled
	int lcd_vcin_reg_en;       //shinjg p13447 samsung oled	
       int bl_en_gpio;               //p10443 ejkim77 sharp lcd
	int bl_swire_gpio;          //p10443 ejkim77 sharp lcd (WS20~)
	int lcd_vddio_reg_en;       //p10443 ejkim77 sharp lcd (WS20~)
	int lcd_vddio_switch_chip_en;       //p10443 ejkim77 sharp lcd (WS20~)	
		int lcd_vci_reg_en;     //p13832_ jiseunghwa
#ifdef CONFIG_MACH_MSM8974_NAMI
	int lcd_vci_reg_en_p;
	int lcd_vci_reg_en_n;
	int lcd_on_ckeck;
#endif
