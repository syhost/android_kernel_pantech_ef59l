...
#ifdef CONFIG_QUALCOMM_BUGFIX_MIRACAST_RESET
	static int hint_state=MDP_WRITEBACK_MIRROR_OFF;
#endif
...
#ifdef CONFIG_QUALCOMM_BUGFIX_MIRACAST_RESET
	hint_state=MDP_WRITEBACK_MIRROR_OFF;
#endif
	mdp5_data->wb = wb;
error:
...
int mdss_mdp_wb_kickoff(struct msm_fb_data_type *mfd)
{
	...
	if (!ctl->power_on)
		return 0;
#ifdef CONFIG_QUALCOMM_BUGFIX_MIRACAST_RESET
	if(hint_state == MDP_WRITEBACK_MIRROR_OFF)
	{
		pr_err("MDP_WRITEBACK_MIRROR_OFF\n");
		return 0;
	}
#endif
	mutex_lock(&mdss_mdp_wb_buf_lock);
