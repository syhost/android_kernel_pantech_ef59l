					  ...
					  struct mdp_overlay *req)
{
	...
	if (req->flags & MDP_DEINTERLACE) {
		...
		rot->src_rect.h /= 2;
#ifdef CONFIG_QUALCOMM_BUGFIX_INTERLACED_GREEN
 		rot->src_rect.y /= 2;
#endif		
	}
...
#ifndef CONFIG_F_SKYDISP_FIXGPU_FALLBACK_ERROR
...
#ifdef CONFIG_QUALCOMM_BUGFIX_MDP_IOMMU_PAGEFAULT
/**
...
#ifdef CONFIG_QUALCOMM_MDP_IOMMU_PAGEFAULT_PATCH
...
#endif
			mutex_unlock(&mfd->lock);
#ifndef CONFIG_QUALCOMM_MDP_IOMMU_PAGEFAULT_PATCH
			mdss_mdp_mixer_pipe_unstage(pipe);
#endif
			mdss_mdp_pipe_unmap(pipe);
		}
				  ...
				  struct msmfb_overlay_data *req)
{
	...
	if ((pipe->src_fmt->is_yuv) &&
		...
		base = pipe->base + MDSS_MDP_REG_VIG_QSEED2_SHARP;
		
		pipe->pp_cfg.sharp_cfg.strength = sharpness_count;
		pipe->pp_cfg.sharp_cfg.edge_thr = 112;
		pipe->pp_cfg.sharp_cfg.smooth_thr = 8;
		pipe->pp_cfg.sharp_cfg.noise_thr = 2;
		
		writel_relaxed(pipe->pp_cfg.sharp_cfg.strength, base);
		base += 4;
		writel_relaxed(pipe->pp_cfg.sharp_cfg.edge_thr, base);
		base += 4;
		writel_relaxed(pipe->pp_cfg.sharp_cfg.smooth_thr, base);
		base += 4;
		writel_relaxed(pipe->pp_cfg.sharp_cfg.noise_thr, base);
	}
	//end
...
#ifdef CONFIG_QUALCOMM_MP3_LCDOFF_CURRENT_PATCH
...
#ifndef CONFIG_QUALCOMM_MP3_LCDOFF_CURRENT_PATCH
...
#ifndef CONFIG_QUALCOMM_BUGFIX_MIRACAST_RESET		
