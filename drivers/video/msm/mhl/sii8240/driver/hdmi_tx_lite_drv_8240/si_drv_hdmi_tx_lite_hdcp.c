...
#include <linux/random.h> //<ZORO_MHL>

unsigned int hdcp_cts_1B03 = 0; //<ZORO_MHL>
...
/*<ZORO_MHL> start*/
unsigned int hdcp_wait1_ms = 200;
unsigned int hdcp_wait1_ms_random = 0;
unsigned int hdcp_wait2_ms = 200;
unsigned int hdcp_wait2_ms_random = 0;
unsigned int hdcp_wait3_ms = 0;
unsigned int hdcp_wait3_ms_random = 0;

void HDCP_Wait1(void)
{
	unsigned int wait; //<ZORO_MHL>

	wait = hdcp_wait1_ms;

	if (hdcp_wait1_ms_random) {
		wait = wait + get_random_int() % hdcp_wait1_ms_random + 1;
	}
	pr_err("HDCP Wait1:%u\n", wait); //<ZORO_MHL>
	if (wait) {
		HalTimerWait(wait);
...
void HDCP_Wait2(void)
{
	unsigned int wait; //<ZORO_MHL>

	wait = hdcp_wait2_ms;

	if (hdcp_wait2_ms_random) {
		wait = wait + get_random_int() % hdcp_wait2_ms_random + 1;
	}
	pr_err("HDCP Wait2:%u\n", wait); //<ZORO_MHL>
	if (wait) {
		HalTimerWait(wait);
...
void HDCP_Wait3(void)
{
	unsigned int wait; //<ZORO_MHL>

	wait = hdcp_wait3_ms;

	if (hdcp_wait3_ms_random) {
		wait = wait + get_random_int() % hdcp_wait3_ms_random + 1;
	}
	pr_err("HDCP Wait3:%u\n", wait); //<ZORO_MHL>
	if (wait) {
		HalTimerWait(wait);
...
#endif //)
		/*<ZORO_MHL>start*/
		/* porting from android-ics-driver-for-8240-ver-1-00-00 */
		if (hdcp_cts_1B03) {
			/*
			...
			*/
			if (HDCP_REPEATER_YES== (HDCP_REPEATER_MASK & queryData)){
				HalTimerWait(5000);
			}
		}
		/*<ZORO_MHL>end*/
