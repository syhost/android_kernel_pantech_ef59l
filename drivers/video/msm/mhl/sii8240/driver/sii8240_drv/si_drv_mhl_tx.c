...
#include "si_edid.h"
//<ZORO_MHL>#define SILICON_IMAGE_ADOPTER_ID 322
#define PMC_ADOPTER_ID 1668 //<ZORO_MHL>
...
static int mhl_mode;
static bool_t is_discovering = false;
static bool_t is_mhl_connected = false;
//extern void hdmi_common_set_hpd(int on);
extern int pm8921_set_usb_power_supply_type(enum power_supply_type type); //<ZORO_MHL>^M
extern void pm8921_charger_vbus_draw(unsigned int mA); //<ZORO_MHL>

void mhl_sync_detect_off(void);
extern void HDCP_Wait1(void);
extern void HDCP_Wait2(void);
extern void HDCP_Wait3(void);

extern spinlock_t mhl_connection_flag_lock;

static int ftm_in;
static int ftm_detect_intr;
static struct completion ftm_intr_done;

uint8_t rgndImpedance;

#ifdef MHL_ENABLE_FTM
unsigned int retry1_timeout_hz = HZ * 2;
unsigned int retry1_max = 0;
//#error "[DEBUG]MHL_FTM_BUILD"
#else
unsigned int retry1_timeout_hz = HZ * 2;
unsigned int retry1_max = 1;
//#error "[DEBUG]MHL_NORMAL_BUILD"
#endif
#ifdef CONFIG_MHL_RETRY_FULL
unsigned int retry2_max = 150;
#else
unsigned int retry2_max = 50;
#endif
uint8_t pollIntervalMs = EVENT_POLL_INTERVAL_30_MS;
/* <ZORO_MHL> */
unsigned int retry_interval_ms = 200;

unsigned int edid_wait1_ms = 0; //<ZORO_MHL>
unsigned int edid_wait2_ms = 0; /*<ZORO_MHL>*/
static struct completion ftm_intr_done;
...
void SiiMhlTxDrvApplyColorSpaceSettings( void)
{
    ...
    if (packedPixelStatus)
	{
        ...
        SiiRegModify(REG_MHLTX_CTL4,(BIT_DATA_SWING_CTL_MASK | BIT_CLK_SWING_CTL_MASK),0x3F);  //ZORO_MHL>
    ...
    else
    {
        ...
        SiiRegModify(REG_MHLTX_CTL4,(BIT_DATA_SWING_CTL_MASK | BIT_CLK_SWING_CTL_MASK),0x33);  //ZORO_MHL>
...
void SiiMhlTxDrvAcquireUpstreamHPDControlDriveLowImpl ( void )
{
    ...
    SiiRegModify(REG_INTR5_MASK, BIT_INTR5_CKDT_CHANGE, 0x00); /*<ZORO_MHL>*/    
    setupstream_hpd(upStreamHPD);// NAMI
}
...
void SiiMhlTxDrvAcquireUpstreamHPDControlDriveHighImpl ( void )
{
    ...
    SiiMhlTxNotifyUpStreamHPD(upStreamHPD);
    SiiRegModify(REG_INTR5_MASK, BIT_INTR5_CKDT_CHANGE, BIT_INTR5_CKDT_CHANGE); /*<ZORO_MHL>*/
...
#ifdef PROCESS_INTR1 //(
uint8_t regIntr1;
  TXD_DEBUG_PRINT(("[MHL] Int1Isr Called\n")); //<ZORO_MHL>
...
//  TXD_DEBUG_PRINT(("[MHL] HdcpIsr Called\n")); //<ZORO_MHL>
...
*/
	...
	ERROR_DEBUG_PRINT(("Drv: SiiMhlTxChipInitialize:"
 	...
 	// setup device registers. Ensure RGND interrupt would happen.
#endif//)
	if (!is_discovering)	//<ZORO_MHL>
		WriteInitialRegisterValuesPartTwo(0);
	else
		WriteInitialRegisterValuesPartTwo(1);//<ZORO_MHL>
...
#ifdef INTERLEAVE_INT7AND8 //(
void Int7And8Isr( void )
{
uint8_t statusIntr7;
    statusIntr7 = SiiRegRead(REG_INTR7);
    if (statusIntr7)
    {
		INFO_DEBUG_PRINT(("Got INTR7 = %02X\n", (int)statusIntr7));
...
uint8_t cbusStatus;
    ...
    if (BIT_CBUS_HPD & cbusStatus)
    {
    	...
    	HalTimerWait(edid_wait2_ms); /*<ZORO_MHL>*/
...
char *szFormats[]=
{
    ...
    , "\tREG_INTR1                  0x%02x\n"
    , "\tREG_INTR1_MASK             0x%02x\n"
    , "\tREG_INTR2                  0x%02x\n"
    , "\tREG_INTR2_MASK             0x%02x\n"
    , "\tREG_INTR3                  0x%02x\n"
    , "\tREG_INTR3_MASK             0x%02x\n"
    , "\tREG_INTR4                  0x%02x\n"
    , "\tREG_INTR4_MASK             0x%02x\n"
    , "\tREG_INTR5                  0x%02x\n"
    , "\tREG_INTR5_MASK             0x%02x\n"
    , "\tREG_INTR7                  0x%02x\n"
    , "\tREG_INTR7_MASK             0x%02x\n"
    , "\tREG_INTR8                  0x%02x\n"
    , "\tREG_INTR8_MASK             0x%02x\n"
    , "\tREG_INTR9                  0x%02x\n"
    , "\tREG_INTR9_MASK             0x%02x\n"
    , "\tREG_CBUS_INT_0             0x%02x\n"
    , "\tREG_CBUS_INT_0_MASK        0x%02x\n"
    , "\tREG_CBUS_INT_1             0x%02x\n"
    , "\tREG_CBUS_INT_1_MASK        0x%02x\n"
    , "\tREG_TPI_INTR_ST0           0x%02x\n"
    , "\tREG_TPI_INTR_ST0_ENABLE    0x%02x\n"
};
uint8_t intStatus[sizeof(szFormats)/sizeof(char *)];
    intStatus[i++] =SiiRegRead(REG_INTR_STATE          );
    intStatus[i++] =SiiRegRead(REG_INTR1               );
    intStatus[i++] =SiiRegRead(REG_INTR1_MASK          );
    intStatus[i++] =SiiRegRead(REG_INTR2               );
    intStatus[i++] =SiiRegRead(REG_INTR2_MASK          );
    intStatus[i++] =SiiRegRead(REG_INTR3               );
    intStatus[i++] =SiiRegRead(REG_INTR3_MASK          );
    intStatus[i++] =SiiRegRead(REG_INTR4               );
    intStatus[i++] =SiiRegRead(REG_INTR4_MASK          );
    intStatus[i++] =SiiRegRead(REG_INTR5               );
    intStatus[i++] =SiiRegRead(REG_INTR5_MASK          );
    intStatus[i++] =SiiRegRead(REG_INTR7               );
    intStatus[i++] =SiiRegRead(REG_INTR7_MASK          );
    intStatus[i++] =SiiRegRead(REG_INTR8               );
    intStatus[i++] =SiiRegRead(REG_INTR8_MASK          );
    intStatus[i++] =SiiRegRead(REG_INTR9               );
    intStatus[i++] =SiiRegRead(REG_INTR9_MASK          );
    intStatus[i++] =SiiRegRead(REG_CBUS_INT_0          );
    intStatus[i++] =SiiRegRead(REG_CBUS_INT_0_MASK     );
    intStatus[i++] =SiiRegRead(REG_CBUS_INT_1          );
    intStatus[i++] =SiiRegRead(REG_CBUS_INT_1_MASK     );
    intStatus[i++] =SiiRegRead(REG_TPI_INTR_ST0        );
    intStatus[i++] =SiiRegRead(REG_TPI_INTR_ST0_ENABLE );

    TXD_DEBUG_PRINT(("Drv: %s:%d %stranscode\n",pszFile,iLine,(mfdrvTranscodeMode & g_modeFlags)?"":"non-"));
    TXD_DEBUG_PRINT(("\tDrv: REG_DISC_CTRL1:%02x\n",(uint16_t)SiiRegRead(REG_DISC_CTRL1)));
#ifdef FPGA_DEBUG //(
    TXD_DEBUG_PRINT(("Drv: REG_TPI_DEBUG_STATUS_1:%02x\n",(uint16_t)SiiRegRead( REG_TPI_DEBUG_STATUS_1)));
    TXD_DEBUG_PRINT(("Drv: REG_TPI_DEBUG_STATUS_2:%02x\n",(uint16_t)SiiRegRead( REG_TPI_DEBUG_STATUS_2)));
    TXD_DEBUG_PRINT(("Drv: REG_TPI_DEBUG_STATUS_3:%02x\n",(uint16_t)SiiRegRead( REG_TPI_DEBUG_STATUS_3)));
    TXD_DEBUG_PRINT(("Drv: REG_TPI_DEBUG_STATUS_4:%02x\n",(uint16_t)SiiRegRead( REG_TPI_DEBUG_STATUS_4)));
    TXD_DEBUG_PRINT(("Drv: REG_TPI_DEBUG_STATUS_5:%02x\n",(uint16_t)SiiRegRead( REG_TPI_DEBUG_STATUS_5)));
    TXD_DEBUG_PRINT(("Drv: REG_TPI_DEBUG_STATUS_6:%02x\n",(uint16_t)SiiRegRead( REG_TPI_DEBUG_STATUS_6)));
    TXD_DEBUG_PRINT(("Drv: REG_TPI_DEBUG_STATUS_7:%02x\n",(uint16_t)SiiRegRead( REG_TPI_DEBUG_STATUS_7)));
#endif //)
    for ( i=0; i < sizeof(intStatus)/sizeof(intStatus[0]); ++i)
    {
        if (intStatus[i])
        {
            SiiOsDebugPrintAlwaysShort(szFormats[i],(uint16_t)intStatus[i]);
        }
    }
}
#define DumpInterruptStatus() DumpInterruptStatusImpl(__FILE__,__LINE__)
#else //)(
#define DumpInterruptStatus()
#endif //)
...
void SiiMhlTxDeviceIsr (void)
{
	...
	{
#endif //)
    		TXD_DEBUG_PRINT(("[MHL] SiiMhlTxDeviceIsr Called\n")); //<ZORO_MHL>
		if (ftm_in) {	/*<ZORO_MHL>*/
			ftm_detect_intr++;	/*<ZORO_MHL>*/
			complete(&ftm_intr_done);	/*<ZORO_MHL>*/
		}	/*<ZORO_MHL>*/
		DumpInterruptStatus();
...
void SiiMhlTxDrvTmdsControl (bool_t enable)
{
	...
	if( enable )
	{
		...
		AppNotifyMhlEvent(MHL_TX_EVENT_HDCP_PASS, 0, NULL); /*<ZORO_MHL>*/
...
void SiiMhlTxDrvNotifyEdidChange (void)
{
    ...
    else
    {
    	...
    	HalTimerWait(edid_wait1_ms); //<ZORO_MHL>
    }
}
//------------------------------------------------------------------------------
...
static void WriteInitialRegisterValuesPartTwo (uint8_t enableDiscovery)
{
	TXD_DEBUG_PRINT(("Drv: WriteInitialRegisterValuesPartTwo[%d]\n",enableDiscovery));
	InitCBusRegs();
	...
	TXD_DEBUG_PRINT(("Drv: WriteInitialRegisterValuesPartTwo Out \n"));
}
...
uint8_t RgndResultIsMhl (void)
{
	...
	{
//<ZORO_MHL>	uint8_t rgndImpedance;
...
//<ZORO_MHL> start
#if 0
		mhl_mode = MHL_DISCOVERY_RESULT_MHL;
...
void SwitchToD3 (void)
{
	if(POWER_STATE_D3 != fwPowerState)
	{
		TXD_DEBUG_PRINT(("Drv: Switch To D3: pinAllowD3 = %d\n",(int) PlatformGPIOGet(pinAllowD3) ));
...
//<ZORO_MHL> start
        pr_err("%s MHL_EST", __func__); //<ZORO_MHL>
		mhl_mode = MHL_DISCOVERY_RESULT_MHL;
		if (notify_usb_online)
			notify_usb_online(1);
		complete(&rgnd_done);
//<ZORO_MHL> end
        		...
        		if(int4Status & BIT_INTR4_NON_MHL_EST)
        		{
                    ...
                    STROBE_POWER_ON
                        ...
                        SiiRegWrite(REG_INTR4,int4Status);
#endif //)
					DISABLE_DISCOVERY	//<ZORO_MHL>
        			SwitchToD3();
					mhl_mode = MHL_DISCOVERY_RESULT_RETRY; //<ZORO_MHL>
					complete(&rgnd_done); //<ZORO_MHL>
        			return I2C_INACCESSIBLE;
        		}
            }
            else if (POWER_STATE_D0_MHL == fwPowerState)
...
//) FP1798
	...
	///pm8921_set_usb_power_supply_type(POWER_SUPPLY_TYPE_USB_ACA); //<ZORO_MHL>^M NAMI.
	///pm8921_charger_vbus_draw(500);	//<ZORO_MHL> NAMI
}
...
//<ZORO_MHL> start
	...
	spin_lock_irqsave(&mhl_connection_flag_lock, flag);
	is_mhl_connected = false;
	spin_unlock_irqrestore(&mhl_connection_flag_lock, flag);
...
/*<ZORO_MHL> start*/
            ...
            {
                TXD_DEBUG_PRINT(("CBUS:: CMD ABORTED.\n"));
...
{
    if (SiiOsDebugChannelIsEnabled(SII_OSAL_DEBUG_EDID_DBG) || override)
    {
	uint8_t j;
        CachedRegModify(REG_EDID_CTRL
                ,BIT_EDID_CTRL_FIFO_SELECT_MASK
...
void DumpEdidBlockImpl(char *pszFile, int iLineNum,uint8_t override,uint8_t *pData,uint16_t length)
{
    if (SiiOsDebugChannelIsEnabled(SII_OSAL_DEBUG_EDID_DBG)||override)
    {
    uint16_t i;
        SiiOsDebugPrintAlwaysShort("%s:%d EDID DATA:\n",pszFile,iLineNum);
    	for (i = 0; i < length; )
    	{
        uint16_t j;
        uint16_t temp = i;
...
SiiMhlTxDrvGetEdidBlockResult_e SiiMhlTxDrvGetEdidBlock(uint8_t *pBufEdid,uint8_t blockNumber,uint8_t blockSize)
{
    ...
    DumpEdidBlock(0,pBufEdid,blockSize)  // no semicolon needed here
	return gebSuccess;
#else //)(
...
uint8_t offset = EDID_BLOCK_SIZE * (edidFifoBlockNumber & 0x01);
    ...
    DumpEdidBlock(0,pBufEdid,EDID_BLOCK_SIZE)  // no semicolon needed here
...
uint8_t cbusStatus;
    ...
    if (BIT_CBUS_HPD & cbusStatus)
    {
        DumpEdidBlock(0,pEDID,length)  // no semicolon needed here
    	SiiMhlTxDrvAcquireUpstreamHPDControlDriveLow();
        CachedRegModify(REG_EDID_CTRL
           ,(BIT_EDID_CTRL_FIFO_SELECT_MASK | BIT_EDID_CTRL_EDID_MODE_EN_MASK)
           ,(BIT_EDID_CTRL_FIFO_SELECT_EDID | BIT_EDID_CTRL_EDID_MODE_EN_ENABLE)
...

{
	...
	if(PlatformGPIOGet(pinAllowD3) == 0 || (POWER_STATE_D3 == fwPowerState )) {
		HalGpioUsbSw(1);
retry_again:
		if (retry && retry_interval_ms) {
			HalTimerWait(retry_interval_ms);
		}
		is_discovering = true;
		...
		timeout = wait_for_completion_interruptible_timeout
			(&rgnd_done, retry1_timeout_hz);
		if (!timeout) {
			/* most likely nothing plugged in USB */
			/* USB HOST connected or already in USB mode */
			pr_info("Timedout Returning from discovery mode\n");
			if (retry < retry1_max) {
				retry++;
				pr_info("Try Again discovery %d\n", retry);
				goto retry_again;
			}
			pr_err("MHL Discovery retry over\n");

			*result = MHL_DISCOVERY_RESULT_USB;
		} else {
			if (mhl_mode == MHL_DISCOVERY_RESULT_RETRY) {
					if (retry < retry2_max) {
						retry++;
						pr_info("Try Again discovery2 %d\n", retry);
						pr_err("MHL Discovery result rgndImpedance:%u\n", rgndImpedance);
						goto retry_again;
					} else {
						*result = MHL_DISCOVERY_RESULT_USB;
						pr_err("MHL Discovery retry over2\n");
					}
			} else {
				*result = mhl_mode;
			}
	...
	if (*result != MHL_DISCOVERY_RESULT_MHL) {
		spin_lock_irqsave(&mhl_connection_flag_lock, flag);
		is_mhl_connected = false;
		spin_unlock_irqrestore(&mhl_connection_flag_lock, flag);
		HalAcquireIsrLock();
		...
		/* MHL mode and set HPD on */
		spin_lock_irqsave(&mhl_connection_flag_lock, flag);
		is_mhl_connected = true;
		spin_unlock_irqrestore(&mhl_connection_flag_lock, flag);
	}
...
bool is_packed_pixel_available(void)
{
	if (pp_enable == 0)
		return 0;

	return IsMhlPackedPixelAvailable();
}
EXPORT_SYMBOL(is_packed_pixel_available);

bool get_mhl_connection_status(void)
{
	pr_debug("%s : MHL connection status(%s)\n", __func__, is_mhl_connected ? "connected" : "disconnected");
	return is_mhl_connected;
}
EXPORT_SYMBOL(get_mhl_connection_status);

bool ftm_mode_enter(void)
{
	int timeout ;
	int ret;
	...
	init_completion(&ftm_intr_done);
	ftm_detect_intr = 0;
	ftm_in = 1;

	disable_irq(gMhlDevice.pI2cClient->irq);
	ret = SiiMhlTxChipInitialize();
	pr_debug("SiiMhlTxChipInitialize: %d\n",ret);
	enable_irq(gMhlDevice.pI2cClient->irq);

	timeout = wait_for_completion_interruptible_timeout
		(&ftm_intr_done, HZ/2);
	if (!timeout) {
		pr_info("Timedout Returning from MHL_INT Detect\n");
		ftm_detect_intr = 0;
	}
	pr_info("Detect MHL_INT: %d\n",ftm_detect_intr);

	pr_debug("%s: end\n",__func__);
	return ftm_detect_intr;
}

void ftm_mode_leave(void)
{
	pr_debug("%s: begin\n",__func__);
