...
//<ZORO_MHL> start
/// #include <linux/cfgdrv.h> ///NAMI.
void ftm_connect_count(void);
static void do_ftm_connect_count(void);

int force_color_mode = 1;
...
//<ZORO_MHL>static bool_t SiiMhlTxClrDCapRdy( void );
...
uint8_t checkSum=0;
	...
	if(0 == pAviInfoFrame->payLoad.hwPayLoad.namedIfData.ifData_u.bitFields.VIC.VIC)
	{
        ERROR_DEBUG_PRINT(("ERROR: VIC = 0\n"));
    ...
    else
    {
        if (SiiMhlTxReadyForVideo())
        {
            if (SiiMhlTxAttemptPackedPixel())
            {
                ...
                SiiMhlTxDrvTmdsControl( true );
#ifdef INFO_FRAMES_AT_TMDS_ENABLE //(
...
#if 1 //<ZORO_MHL> start
				TXC_DEBUG_PRINT (("MhlTx:MhlTxProcessEvents MHL_MSC_MSG_RCP\n"));
				if((MHL_LOGICAL_DEVICE_MAP & rcpSupportTable [mhlTxConfig.mscMsgData & 0x7F] ) &&
					(shmhl_rcp_key_support_judge(mhlTxConfig.mscMsgData)))
				{
#else /* <ZORO_MHL> end */
				if(MHL_LOGICAL_DEVICE_MAP & rcpSupportTable [mhlTxConfig.mscMsgData & 0x7F] )
				{
...
#if 1 //<ZORO_MHL> start
					shmhl_input_rcp_key(mhlTxConfig.mscMsgData);

					SiiMhlTxRcpkSend(mhlTxConfig.mscMsgData);
#endif /* <ZORO_MHL> end */
				}
				else
				{
...
uint16_t hTotal, vTotal;
    ...
    INFO_DEBUG_PRINT(("VIC was zero!!! hTotal: 0x%04x vTotal:0x%04x\n",hTotal,vTotal));
    return retVal;
}

#ifdef	NEVER /*<ZORO_MHL>*/
...
int timingInfoSource=__LINE__;
	...
	//<ZORO_MHL> start
	if (force_color_mode == 1) {
		inputClrSpc = acsRGB;
	}
	else if (force_color_mode == 2) {
		inputClrSpc = acsYCbCr444;
	}
	else if (force_color_mode == 3) {
...
union
    ...
    DumpEdidBlock(0,pEdidBlock0,sizeof(*pEdidBlock0))  // no semicolon needed here
    ...
    DumpEdidBlock(0,pCeaExtension,sizeof(*pCeaExtension))  // no semicolon needed here
    ...
    uint8_t limit2dVIC = mhlTxConfig.pVideoDataBlock2D[0]->header.lengthFollowingHeader;
        ...
        if (mhlTxConfig.pHdmiVendorSpecificDataBlock != NULL) {
            mhlTxConfig.pHdmiVendorSpecificDataBlock->header.lengthFollowingHeader -= num3DStructureBytesPruned;
        } else {
            ERROR_DEBUG_PRINT(("[ZORO]mhlTxConfig.pHdmiVendorSpecificDataBlock is NULL\n"));
        }
        /* <ZORO_MHL> end */
    ...
    if (mhlTxConfig.pVideoDataBlock2D[0]) //<ZORO_MHL>
    {
        ...
        uint8_t innerLoopLimit = mhlTxConfig.pVideoDataBlock2D[vdb_index]->header.lengthFollowingHeader;
            ...
            uint8_t outerLoopLimit = innerLoopLimit-1;
                ...
                uint8_t *pbSrc= (uint8_t *)&mhlTxConfig.pVideoDataBlock2D[vdb_index]->shortDescriptors[mhlTxConfig.pVideoDataBlock2D[vdb_index]->header.lengthFollowingHeader];
                    ...
                    //size_t stSrc;
                      //  stSrc = pbSrc;
                        TX_PRUNE_PRINT(("moving data up pointer  size:%u %x(0x%02x) <- %x(0x%02x)\n"
...
//re-visit uint8_t edidLimit = mhlTxConfig.pByte13ThroughByte15->byte14.HDMI_3D_LEN;
    ...
    SCRPAD_DEBUG_PRINT(("burstEntryCount3D_VIC: %bd\n",mhlTxConfig.burstEntryCount3D_VIC));
    ...
    if (mhlTxConfig.burstEntryCount3D_VIC < mhlTxConfig.pVideoDataBlock2D[mhlTxConfig.video_data_block_index]->header.lengthFollowingHeader)
    {
        ...
        uint8_t thisModeDoable=0;
            ...
            if (VIC)
            {
                SCRPAD_DEBUG_PRINT(("short Descriptor[%bd] 3D VIC: %bd %s %s %s\n"
                            ,mhlTxConfig.burstEntryCount3D_VIC
                            ,VIC
...
static void SiiMhlTxParseStandardTiming(PEDID_Block0_t pEdidBlock0)
{
    ...
    for (i = 0; i < sizeof(pEdidBlock0->standardTimings)/sizeof(pEdidBlock0->standardTimings[0]); i += 2)
    {
        ...
        uint16_t vertActive=0; //<ZORO_MHL>
...
static bool_t SiiMhlTxCheckEDID_Header(PEDID_Block0_t pEdidBlock0)
{
	...
	if (0 != pEdidBlock0->headerData[EDID_OFFSET_HEADER_FIRST_00])
	{
        ...
        DumpEdidBlock(1,pEdidBlock0,sizeof(*pEdidBlock0))
		return false;
	}
    ...
    if (SiiMhlTxDrvGetEdidFifoNextBlock(pbData))
    {
        if (0 == blockNumber)
        {
        	if (!SiiMhlTxCheckEDID_Header(pEdidBlock0))
        	{
                ...
                DumpEdidBlock(1,pbData,sizeof(*pEdidBlock0))  // no semicolon needed here
    	...
    	if (!SiiMhlTxDoEDID_Checksum(pbData))
    	{
            ...
            DumpEdidBlock(1,pbData,EDID_BLOCK_SIZE)  // no semicolon needed here
                                        ...
                                        )
{
    ...
    if (EDID_EXTENSION_TAG != pCeaExtension->tag)
    {
        ...
        DumpEdidBlock(1,pEdidBlockData,sizeof(*pCeaExtension))
                ...
                case dbtcVideoDataBlock:
                    {
                        ...
                        if (mhlTxConfig.num_video_data_blocks < NUM_VIDEO_DATA_BLOCKS_LIMIT)
                        {
                            ...
                            TX_EDID_PRINT(("EDID -> %bd descriptors in Video Data Block[%d]\n",i,(uint16_t)mhlTxConfig.num_video_data_blocks));
...
PCeaExtension_t pCeaExtension = (PCeaExtension_t)pEdidBlockData;
    ...
    mhlTxConfig.pByte13ThroughByte15=NULL; //<ZORO_MHL>
...
void	SiiMhlTxMscCommandDone( uint8_t data1 )
{
    ...
    else if (MHL_READ_EDID_BLOCK == mhlTxConfig.mscLastCommand)
    {
        ...
        uint8_t Result = EDID_OK;
        	...
        	{
                DumpEdidBlock(0,(uint8_t *)pEdidBlock0,EDID_BLOCK_SIZE)
                EDID_Data.sinkType = sink_type_DVI;
                SiiMhlTxDrvSetOutputColorSpace(BIT_EDID_FIELD_FORMAT_DVI_TO_RGB );
...
#ifdef INTERRUPT_DRIVEN_EDID /*<ZORO_MHL>*/
        if (mhlTxConfig.flags.edidLoopActive)
		{
...
PInfoFrameHeader_t pHeader = (PInfoFrameHeader_t)pInfoFrame;
uint8_t length2 = pHeader->length + sizeof(*pHeader);
    ...
    SiiOsDebugPrint(pszFile,iLine,SII_OSAL_DEBUG_TX, "MhlTx: %s: length:0x%02x -- ",pszId,(uint16_t)length);
	if (SiiOsDebugChannelIsEnabled(SII_OSAL_DEBUG_TX))
	{
		for (j = 0; j < length; j++)
		{
			SiiOsDebugPrintSimpleAlways("%02x ", (uint16_t) *pData++);
			if (AT_ROW_END(j,32))
			{
				SiiOsDebugPrintSimpleAlways("\n");
			}
...
static int SiiMhlTxProcessAviInfoFrameChange(PAVIInfoFrame_t pAviInfoFrame)
{
    ...
    {
#ifdef	NEVER
        INFO_DEBUG_PRINT(("AVI valid: VIC: %d\n",(uint16_t)pAviInfoFrame->payLoad.hwPayLoad.namedIfData.ifData_u.bitFields.VIC.VIC));
        if (0 != aviInfoFrameCmp(&mhlTxConfig.currentAviInfoFrame,pAviInfoFrame))
#endif // NEVER
        {
        ...
        {
            ERROR_DEBUG_PRINT(("MhlTx:  AVI-IF no change\n"));
        }
#endif // NEVER
    }
#ifdef	NEVER
    ...
    {
        ERROR_DEBUG_PRINT(("valid info frame sanity check\n"));
    }
#endif	//	NEVER
    return 0;
}
...
#ifndef INFO_FRAMES_AT_TMDS_ENABLE //(	
    SiiHmdiTxLiteDrvSendHwAviInfoFrame( );
    //SendAudioInfoFrame();
#endif //)
...
/*<ZORO_MHL> add Start*/
bool_t IsMhlPackedPixelAvailable( void )
{
	if (PackedPixelAvailable)
		return true;
	else
...
void ftm_connect_count(void)
{
	...
	pr_debug("[MHL_FTM] %s: start\n", __func__);
	ret = cfgdrv_read(D_HCM_E_BOOT_KERNEL_FLG, sizeof(mode), &mode);

	if (ret == D_HCM_NOT_INITIALIZE) {
		pr_info("[MHL_FTM] %s: hcm is not initialized\n", __func__);
		return;
	}
	else if (ret < 0){
		pr_err("[MHL_FTM] cfgdrv_read error:%d, boot kernel flg\n", ret);
		return;
	}

	if (mode != 0x3) { 
		pr_debug("[MHL_FTM] %s: not Mode 3, do nothing\n", __func__);
		return;
	}
#endif
	do_ftm_connect_count();
	pr_debug("[MHL_FTM] %s: end\n", __func__);
}
...
static void do_ftm_connect_count(void)
{
	...
	ret = cfgdrv_read(D_HCM_E_MHL_CONNECT, sizeof(count), &count);
	if (ret < 0) {
		pr_err("[MHL_FTM] cfgdrv_read error:%d, connect count\n", ret);
		return;
	}
	pr_debug("[MHL_FTM] read connect count: %u\n", count);

	count++;

	ret = cfgdrv_write(D_HCM_E_MHL_CONNECT, sizeof(count),&count);
	if (ret < 0) {
		pr_err("[MHL_FTM] cfgdrv_write error: %d, connect count\n", ret);
		return;
	}
	pr_debug("[MHL_FTM] write connect count: %u\n", count);
	...
	pr_info("[MHL_FTM] verify read connect count: %u\n", count);
#endif
#endif
}
