...
/* Added for FACTORY CMD : p14527 */
#define PANTECH_BOARD_TYPE
#endif
#ifdef PANTECH_BOARD_TYPE
...
static short check_board_type(void){
	...
	gpio_direction_input(538 /* 3 */);
	gpio_direction_input(539 /* 4 */);
	gpio_direction_input(543 /* 8 */);

    board_type = (0x1 & __gpio_get_value(538));
    board_type = board_type | (0x1 & __gpio_get_value(539))<<1;
    board_type = board_type | (0x1 & __gpio_get_value(543))<<2;
...
(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	case SKT:
		len  = sprintf(page, "SKT");	break;
	case KT:
	    len  = sprintf(page, "KT");		break;
	case LGT:
		len  = sprintf(page, "LGT");	break;
	default:
		len  = sprintf(page, "NONE");	break;
	}
    return len;
}
