...
/* Quixotism: state about damaged sdcard */
bool abnormal_sd = false;
...
int mmc_attach_sd(struct mmc_host *host)
{
	...
	while (retries) {
		...
		if (err) {
			/* Quixotism: Skip retry about the damaged sdcard */
			if(abnormal_sd){
				printk("mmc: Skip Retry Process because of abnomal_sd=%d\n", abnormal_sd);
				goto err;
			}
			retries--;
			mmc_power_off(host);
			...
			/* Quixotism: Reset state about the damaged sdcard */
			if(abnormal_sd){
				abnormal_sd = false;
				printk("mmc: Set abnormal_sd to %d because of detected sdcard..\n", abnormal_sd);
			}
		}
		break;
	}
...
err:
	...
	/* Quixotism: Set state about the damaged sdcard */
	if(abnormal_sd == false){
		pr_err("%s: error %d whilst initialising SD card\n",
			mmc_hostname(host), err);
		abnormal_sd = true;
		printk("mmc: Set abnormal_sd to %d in mmc_attach_sd.\n", abnormal_sd);
	}
