	...
	for_each_online_cpu(cpu) {
		...
		if (time_before(next_sampling, appointed_at)) {
			...
			if (dbs_info->cur_policy == NULL)
				printk(KERN_ERR "UPDATE SAMPLING INSTANT POLICY NULL\n");
...
static void do_dbs_timer(struct work_struct *work)
{
	...
	mutex_lock(&dbs_info->timer_mutex);
	/* qti tst code start_p16101*/ 
	if(cpu>(int)cpu_online_mask||cpu<(int)0) 
	panic("dbs_info is corrupted"); 
	/* qti tst code end*/ 
	...
	/* qti tst code start_p16101*/ 
	//printk("@@dbs timer scheduled after delay=%d\n",delay); 
	/* qti tst code end*/ 
	queue_delayed_work_on(cpu, dbs_wq, &dbs_info->work, delay);
	mutex_unlock(&dbs_info->timer_mutex);
	if (dbs_info->cur_policy == NULL)
		printk(KERN_ERR "DO_DBS_TIMER POLICY NULL\n");
}
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	...
	if (dbs_info->cur_policy == NULL)
		printk(KERN_ERR "DBS_TIMER_INIT POLICY NULL\n");
}
...
static int dbs_sync_thread(void *data)
{
	...
	while (1) {
		...
		if (policy == NULL)
			printk(KERN_ERR "SYNC_THREAD POLICY NULL\n");
		}
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_STOP:
		...
		if (delayed_work_pending(&this_dbs_info->work))
			printk(KERN_ERR "WORK PENDING AFTER CORE WENT OFFLINE\n");
