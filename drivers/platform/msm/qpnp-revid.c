...
//20130507. MKS. PMIC_hw_revision
#if defined(FEATURE_PANTECH_PMIC_PMIC_HW_REVISION)
static int hw_revision1=0;	// PMIC8941
static int hw_revision2=0;	// PMIC8841
module_param_named(hw_revision1, hw_revision1, int, S_IRUGO);
module_param_named(hw_revision2, hw_revision2, int, S_IRUGO);
#endif
		...
		u8 subtype, u8 rev1, u8 rev2, u8 rev3, u8 rev4)
{
	...
	if (subtype >= ARRAY_SIZE(pmic_names) || subtype == 0)
		...
		pos += snprintf(buf + pos, n - pos, ": %s",
				pmic_names[subtype]);
#if defined(FEATURE_PANTECH_PMIC_PMIC_HW_REVISION)
		if(subtype==1)		// PMIC8941
			hw_revision1 = rev3|(rev4<<8);
		else if(subtype==2)	// PMIC8841
			hw_revision2 = rev3|(rev4<<8);
#endif
	}
