...
#ifdef FEATURE_PANTECH_USB_QXDM_ONOFF
bool qxdm_msg_onoff_control = 0;
#endif
#ifdef FEATURE_PANTECH_USB_BLOCKING_MDMSTATE
...
void mode_switch_cb(int mode)
{
	...
	if(ret < 0){
		printk(KERN_ERR "[PUSB][%s]mode_switch error[%d]\n",__func__, mode);
		return;
	}
	dev_mode_change->pc_mode_switch_flag = ret + 1;
	wake_up_interruptible(&dev_mode_change->device_mode_change_wq);
	printk(KERN_ERR "[PUSB][%s]mode[%d]ret[%d]\n",__func__, mode, ret);
}
...
int usb_reconnect_cb(int type)
{
	...
	if(usb_manager_state_enabled){
		...
		if(type){
			...
			ret = get_func_thru_type(dev_mode_change->g_device_type);
		}
		dev_mode_change->pc_mode_switch_flag = ret + 1;
		wake_up_interruptible(&dev_mode_change->device_mode_change_wq);
		printk(KERN_ERR "[PUSB][%s]index[%d]\n",__func__, ret);
	}
	return 1;
}
...
int type_switch_cb(int type)
{
	...
	ret = get_func_thru_type(type);
	if(ret < 0){
		printk(KERN_ERR "[PUSB][%s]error[0x%x]\n",__func__, type);
		return -1;
	}
	dev_mode_change->pc_mode_switch_flag = ret + 1;
	wake_up_interruptible(&dev_mode_change->device_mode_change_wq);
	printk(KERN_ERR "[PUSB][%s][0x%x]index[%d]\n",__func__, type, ret);
	return 1;
}
	...
	while(functions){
		...
		if(func){
			if(!strcmp(func, "mass_storage")){
				...
				type |= PTP_TYPE_FLAG;
			}else if(!strcmp(func, "audio_source")) { //LS2_USB tarial audio_source for AOA 2.0 audio device
				type |= AUDIO_TYPE_FLAG;
			}else if(!strcmp(func, "qdss")){ //tarial qdss test
				type |= QDSS_TYPE_FLAG;
			}
		}
	}
	...
	if(type){
		...
		if(b_factory_mode_enabled && b_factory_mode_request){
			if(type & ~(ACM_TYPE_FLAG | DIAG_TYPE_FLAG | ADB_TYPE_FLAG) )	
			{
				...
				printk(KERN_ERR "[PUSB][%s] ignored [0x%x] in factory mode.\n", __func__, type);
				return;
			}
		}
#endif
		...
		if(ret <= 0) {
			printk(KERN_ERR "[PUSB][%s] do not need wait_event_interruptible()\n", __func__);
		...
		if (ret < 0) {
			printk(KERN_ERR "[PUSB]android_enabled_function_cb: %d\n", ret);
			return;
		}
	}
	return;
			 ...
			 size_t count, loff_t *ppos)
{
	...
	if (temp_device_type == dev_mode_change->g_device_type) {
		printk(KERN_ERR "[PUSB]%s - Function already configured[0x%x] \n" ,
...
int pantech_vbus_disconnect(void)
{
  ...
  wake_up_interruptible(&dev_mode_change->device_mode_change_wq);
	printk(KERN_ERR "[PUSB][%s]type_mode[0x%x] ret[%d]\n",__func__, DEFAULT_TYPE_FLAG, ret);
