...
#ifdef CONFIG_ANDROID_PANTECH_USB_HIGH_SPEED_ONLY
...
unsigned int mtp_debug_f = 0;
module_param(mtp_debug_f, uint, S_IRUGO | S_IWUSR);
	...
	size_t count, loff_t *pos)
{
	...
	if(mtp_debug_f)printk(KERN_ERR "mtp_read(%d)\n", count);
...
requeue_req:
	...
	if(req){
		...
		if (ret < 0) {
			r = -EIO;
			if(mtp_debug_f) printk(KERN_ERR "[%s]:-EIO[%d]\n",__func__, ret);
			mtp_req_put(dev, &dev->rx_idle_list, req);
			goto done;
		} else {
			DBG(cdev, "rx %p queue\n", req);
	...
	if(mtp_debug_f)printk(KERN_ERR "mtp_read returning %d\n", r);
	return r;
}
...
static void receive_file_work(struct work_struct *data)
{
	...
	if(mtp_debug_f)printk(KERN_ERR "receive_file_work(%lld)\n", count);
		...
		if (write_req) {
			...
			if (ret != write_req->actual) {
				if (dev->state != STATE_OFFLINE){
					dev->state = STATE_ERROR;
					r = -EIO;
					if(mtp_debug_f) printk(KERN_ERR "[%s]:-EIO[%d], actual[%d]\n",__func__, ret, write_req->actual);
				}
	...
	if(mtp_debug_f)printk(KERN_ERR "receive_file_work returning %d\n", r);
}
