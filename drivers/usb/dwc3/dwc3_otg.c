...
#if defined(FEATURE_PANTECH_PMIC_CHARGER_SMB349)
...
#ifdef FEATURE_PANTECH_PMIC_OTG_UVLO
...
extern int HalGpioGetUsbIdPin(void);
#endif
...
#ifdef FEATURE_PANTECH_PMIC_CHARGER_SMB349
...
{
	printk("UVLO!! OTG power disconnect, on = [%d]\n",on);
	is_otg_enabled = 0;
#ifdef CONFIG_ANDROID_PANTECH_USB_OTG_INTENT
	set_otg_dev_state(3);
#endif
...
#endif
#ifndef FEATURE_PANTECH_SIO_BUG_FIX
		if (!test_bit(B_SESS_VLD, &dotg->inputs) ||
