...
#ifdef FEATURE_PANTECH_USB_VER_SWITCH
...
#if (defined(CONFIG_FB_MSM_MDSS_HDMI_MHL_SII8240) && defined (CONFIG_MACH_MSM8974_NAMI))
extern void mhl_start_work_queue(void);
#endif

#ifdef FEATURE_PANTECH_USB_EXTERNAL_ID_PULLUP
#ifdef FEATURE_PANTECH_PMIC_SHARED_DATA
#include <mach/msm_smsm.h>
static int get_oem_hw_rev(void)
{
...
struct dwc3_msm {
	...
	//LS4-USB erratic buffer fail issue
#ifdef FEATURE_PANTECH_SIO_BUG_FIX
...
#ifdef FEATURE_PANTECH_USB_TUNE_SIGNALING_PARAM
...
#if ((defined(T_EF59S) || defined(T_EF59K) || defined(T_EF59L)) && (BOARD_VER > WS10))
		msleep(30);
		//Set OTG off, HSUSB Signaling set hs amplitude 14%
		dwc3_msm_write_readback(mdwc->base, PARAMETER_OVERRIDE_X_REG, 0x00001E00, 10 << 9); 
#endif
	} else {		
 ...
 * SSUSB tune
 */
static unsigned long tx_swing_full = 127;
...
static unsigned long tx_deemph_3_5db = 22;
...
static unsigned long rx_equalization_data = 0x00; //1031 base default value
...
static void dwc3_msm_ss_phy_reg_init(struct dwc3_msm *msm)
{
	...
	data &= ~(0x7 << 8);
#ifdef FEATURE_PANTECH_USB_TUNE_SIGNALING_PARAM
	data |= (rx_equalization_data << 8); //set value 000 ~ 111
#else
	data |= (0x3 << 8);
#endif
	data |= (0x1 << 11);
...
#if 0//def FEATURE_PANTECH_USB_TUNE_SIGNALING_PARAM //LS2_USB tarial 1032A remove this setting
	// Set bit [0] of DWC3_GUSB3PIPECTL ot '0'
	dwc3_msm_write_readback(msm->base, DWC3_GUSB3PIPECTL(0), 0x01, rx_elastic_buffer);
#endif
	...
	data = 0x07f01605;
	data &= ~0x07f03F00;
	data |= tx_deemph_3_5db << 8;
	data |= tx_swing_full << 20;
				  ...
				  union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		//val->intval = mdwc->online;		//20130710, P14787, delete
		//20130710, P14787, usbOnline update only SDP
...
static void dwc3_msm_external_power_changed(struct power_supply *psy)
{
	...
	pr_err("%s:djjeon0617 ret.intval[%d]\n", __func__,ret.intval);
...
#ifndef FEATURE_PANTECH_SIO_BUG_FIX //remove otg id detect state during operating peripheral mode
		mdwc->ext_xceiv.id = mdwc->id_state;
		dwc3_resume_work(&mdwc->resume_work.work);
#else
...
#if (defined(CONFIG_FB_MSM_MDSS_HDMI_MHL_SII8240) && defined (CONFIG_MACH_MSM8974_NAMI))	
	pr_info("dwc3_pmic_id_irq:id=%d\n",id);
	if(id==0)
	{
		///HalGpioUsbSw(1);
		mhl_start_work_queue();
...
#ifdef CONFIG_ANDROID_PANTECH_USB_MANAGER
	INIT_DELAYED_WORK(&msm->connect_work, dwc3_connect_work);
#endif
	...
	if (msm->ext_xceiv.otg_capability) {
		...
		if (msm->pmic_id_irq > 0) {
			...
			if (ret == -EPROBE_DEFER) {
				...
				if(get_oem_hw_rev() >= 1) // in case of EF61,EF62, usb external id set after WS20
#endif
#else
				if(0) // noting to do
#endif /* FEATURE_PANTECH_PMIC_SHARED_DATA */
