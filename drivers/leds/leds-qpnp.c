...
//#define PAN_NEED_KEEP_GPLED_0_ON		// p11309 - 2013.04.15 for individual GPLED Control
...
#endif
//++ p11309 - 2013.03.27 for Qualcomm Patch + Modified
#define GPLED_MAX_LEVEL					LED_FULL
#define GPLED_ROW_SRC_SEL(base)			(base + 0x40)
#define GPLED_ENABLE(base)				(base + 0x46)
#define GPLED_GLOBAL_ROW_SCAN(base)		(base + 0xB1)
#define GPLED_PWM_PER_ADJ_LSB(base)		(base + 0xB3)
#define GPLED_PWM_PER_ADJ_MSB(base)		(base + 0xB4)
#define GPLED_ROW_SRC(base)				(base + 0xE5)
//-- p11309
...
#ifdef CONFIG_F_SKYDISP_KEEP_BACKLIGHT_TO_KERNEL //seunghwa_Ji p13832
static u8 wled_pwm_regs[] = {
	0x40,0x41
};
#endif
...
//++ p11309 - 2013.03.27 for Qualcomm Patch + Modified
/**
 *  gpled_config_data - gpled configuration data
 *  @lut_params - lut parameters to be used by pwm driver
 *  @pwm_device - pwm device
 *  @pwm_channel - pwm channel to be configured for led
 *  @pwm_period_us - period for pwm, in us
...
static int gpled_enabled_port = 0x0;
static int gpled_power_status_flag = 0x0;
//-- p11309
...
//++ p11309 - 2013.03.27 for Qualcomm Patch + Modified
	struct gpled_config_data	*gpled_cfg;
//-- p11309
...
static void qpnp_dump_regs(struct qpnp_led_data *led, u8 regs[], u8 array_size)
{
	int i;
	u8 val;
//  	u16 pwm_enable_register[] = {
//  		0xe400, 0xe500, 0xe600, 0xe700
//  	};
...
// 	printk("===== %s PWM register dump start =====\n", led->cdev.name);
// 	for (i = 0; i < 4; i++) {
// 		spmi_ext_register_readl(led->spmi_dev->ctrl,
// 			led->spmi_dev->sid,
// 			pwm_enable_register[i] + 0x46,
// 			&val, sizeof(val));
// 		printk("0x%x = 0x%x\n", pwm_enable_register[i] + 0x46, val);
// 	}
// 	printk("===== %s PWM register dump end =====\n", led->cdev.name);
}
...
#if (defined(CONFIG_PANTECH_CAMERA) && (BOARD_VER > WS10))//flash
...
static int qpnp_flash_set(struct qpnp_led_data *led)
{
	...
	if (val > 0) {
		...
		if (led->flash_cfg->torch_enable) {
			 ...
			 * by register value going from 0x00 to 0xE0.
			 */
#ifndef CONFIG_PANTECH_CAMERA //flash
			rc = qpnp_led_masked_write(led,
				FLASH_ENABLE_CONTROL(led->base),
				FLASH_ENABLE_MODULE_MASK,
				FLASH_ENABLE_MODULE);
...
//++ p11309 - 2013.03.27 for Qualcomm Patch + Modified
static int qpnp_gpled_set(struct qpnp_led_data *led)
{
	int duty_us;
	int rc;	

#ifdef PAN_NEED_KEEP_GPLED_0_ON
	if ( led->cdev.brightness == 0 ) led->cdev.brightness = 1;
#endif

	if (led->cdev.brightness) {		

		if ( gpled_power_status_flag == 0 ) {
			// GPLED Whole ON
			rc = qpnp_led_masked_write(led, GPLED_ENABLE(led->base), 0x80, 0x80);
			if (rc) {
				dev_err(&led->spmi_dev->dev, "[+++ LED +++] GPLED: Failed to write led enable reg\n");
				return rc;
			}
		}
		gpled_power_status_flag |= (0x1 << (led->id - QPNP_ID_GPLED_0));

		if ( led->gpled_cfg->mode == GPLED_MODE_PWM ) {
			duty_us = (led->gpled_cfg->pwm_period_us * led->cdev.brightness) / GPLED_MAX_LEVEL;
			rc = pwm_config(led->gpled_cfg->pwm_dev, duty_us, led->gpled_cfg->pwm_period_us);
			if (rc < 0) {
				dev_err(&led->spmi_dev->dev, "[+++ LED +++] GPLED: Failed to configure pwm for new values\n");
				return rc;
			}
		}

		rc = pwm_enable(led->gpled_cfg->pwm_dev);
	} else {
		gpled_power_status_flag &= ~(0x1 << (led->id - QPNP_ID_GPLED_0));
		pwm_disable(led->gpled_cfg->pwm_dev);		
	}

// 	printk("[+++ LED +++] %s: id=%d, brightness=%d, power_status=%X \n", 
// 		__func__, led->id, led->cdev.brightness, gpled_power_status_flag);

	// GPLED Whole OFF
	if ( gpled_power_status_flag == 0 ) {

		rc = qpnp_led_masked_write(led, GPLED_ENABLE(led->base), 0x00, 0x00);
		if (rc) {
			dev_err(&led->spmi_dev->dev, "[+++ LED +++] GPLED: Failed to write led disable reg\n");
			return rc;
		}
	}

	qpnp_dump_regs(led, gpled_debug_regs, ARRAY_SIZE(gpled_debug_regs));
...
static int qpnp_rgb_set(struct qpnp_led_data *led)
{
	...
	if (led->cdev.brightness) {
		...
		if (led->rgb_cfg->pwm_cfg->mode == PWM_MODE) {
			...
			if (rc < 0) {
				dev_err(&led->spmi_dev->dev, "RGB: Failed to " \
					"configure pwm for new values\n");
                                //1026 patch dev_err(&led->spmi_dev->dev,
                                //1026 patch        "pwm config failed\n");
				return rc;
			}
		}
		rc = qpnp_led_masked_write(led,
		...
		if (rc) {
			dev_err(&led->spmi_dev->dev,
				"RGB: Failed to write led disable reg\n");
			return rc;
		}
	}
...
//++ p11309 - 2013.03.27 for Qualcomm Patch + Modified
	case QPNP_ID_GPLED_0:
	case QPNP_ID_GPLED_1:
	case QPNP_ID_GPLED_2:
	case QPNP_ID_GPLED_3:
		rc = qpnp_gpled_set(led);
		if (rc < 0)
			dev_err(&led->spmi_dev->dev,
				"GPLED set brightness failed (%d)\n", rc);
		break;
//-- p11309
...
#ifdef CONFIG_F_SKYDISP_KEEP_BACKLIGHT_TO_KERNEL //seunghwa_Ji p13832
	int i;
	unsigned char wled_pwm_reg_data[sizeof(wled_pwm_regs)]={0,};
#endif
	switch (led->id) {
	case QPNP_ID_WLED:
#ifdef CONFIG_F_SKYDISP_KEEP_BACKLIGHT_TO_KERNEL //seunghwa_Ji p13832
		for(i = 0; i < sizeof(wled_pwm_regs); i++){
			mutex_lock(&led->lock);
			spmi_ext_register_readl(led->spmi_dev->ctrl,
					led->spmi_dev->sid,led->base + wled_pwm_regs[i],&wled_pwm_reg_data[i], sizeof(wled_pwm_reg_data[i]));
			mutex_unlock(&led->lock);
		}
		led->cdev.max_brightness  = ((wled_pwm_reg_data[1] & 0xff) << 8) | (wled_pwm_reg_data[0] & 0xff);
#else
...
//++ p11309 - 2013.03.27 for Qualcomm Patch + Modified
	...
	case QPNP_ID_GPLED_2:
	case QPNP_ID_GPLED_3:
		led->cdev.max_brightness = GPLED_MAX_LEVEL;
		break;
//-- p11309
...
//++ p11309 - 2013.03.27 for Qualcomm Patch + Modified
static int __devinit qpnp_gpled_init(struct qpnp_led_data *led)
{
	int rc, start_idx, idx_len;

	printk("[+++ LED +++] %s: Enabled GPLED IDs = %X\n", __func__, gpled_enabled_port);

	/* enable row source select: 0~3 bit, 0-VPh_pwr, 1-VBST */
	rc = qpnp_led_masked_write(led, GPLED_ROW_SRC_SEL(led->base), 0x0, 0x0);
	if (rc) {
		dev_err(&led->spmi_dev->dev, "Enable row src selwrite failed(%d)\n", rc);
		return rc;
	}

	/* enable row source: 0~3 Bit, 0-disable, 1-enable, 7bit - Whole Key */
	rc = qpnp_led_masked_write(led, GPLED_ROW_SRC(led->base), 0x80 + gpled_enabled_port, 0x80 + gpled_enabled_port);
	if (rc) {
		dev_err(&led->spmi_dev->dev, "Enable row src write failed(%d)\n", rc);
		return rc;
	}

	/* enable module: 0bit, 0-diable, 1-Whole Enable */
	rc = qpnp_led_masked_write(led, GPLED_ENABLE(led->base), 0x80, 0x80);
	if (rc) {
		dev_err(&led->spmi_dev->dev, "Enable module write failed(%d)\n", rc);
		return rc;
	}

	/* program pwm period lsb */
	rc = qpnp_led_masked_write(led, GPLED_PWM_PER_ADJ_LSB(led->base), 0x00, 0x00);
	if (rc) {
		dev_err(&led->spmi_dev->dev, "pwm period lsb write failed(%d)\n", rc);
		return rc;
	}

	/* program pwm period msb */
	rc = qpnp_led_masked_write(led, GPLED_PWM_PER_ADJ_MSB(led->base), 0x04, 0x04);
	if (rc) {
		dev_err(&led->spmi_dev->dev, "pwm period msb write failed(%d)\n", rc);
		return rc;
	}

	/* program global row scan */
	rc = qpnp_led_masked_write(led, GPLED_GLOBAL_ROW_SCAN(led->base), 0x80, 0x80);
	if (rc) {
		dev_err(&led->spmi_dev->dev, "global row scan write failed(%d)\n", rc);
		return rc;
	}

	if (led->gpled_cfg->pwm_channel != -1) {
		led->gpled_cfg->pwm_dev =
			pwm_request(led->gpled_cfg->pwm_channel, led->cdev.name);

		if (IS_ERR_OR_NULL(led->gpled_cfg->pwm_dev)) {
			dev_err(&led->spmi_dev->dev, "could not acquire PWM Channel %d, error %ld\n",
				led->gpled_cfg->pwm_channel,
				PTR_ERR(led->gpled_cfg->pwm_dev)
			);
			led->gpled_cfg->pwm_dev = NULL;
			return -ENODEV;
		}

		if (led->gpled_cfg->mode == GPLED_MODE_LPG) {
			start_idx = led->gpled_cfg->duty_cycles->start_idx;
			idx_len = led->gpled_cfg->duty_cycles->num_duty_pcts;

			if (idx_len >= PWM_LUT_MAX_SIZE && start_idx) {
				dev_err(&led->spmi_dev->dev, "Wrong LUT size=%d or index=%d\n", idx_len, start_idx);
				return -EINVAL;
			}
			if ((start_idx + idx_len) > PWM_LUT_MAX_SIZE) {
				dev_err(&led->spmi_dev->dev, "Exceed LUT limit, sz=%d, idx=%d\n", idx_len, start_idx);
				return -EINVAL;
			}
			rc = pwm_lut_config(led->gpled_cfg->pwm_dev,
				led->gpled_cfg->pwm_period_us,
				led->gpled_cfg->duty_cycles->duty_pcts,
				led->gpled_cfg->lut_params);
			if (rc < 0) {
		...
		dev_err(&led->spmi_dev->dev, "GPLED: Invalid PWM channel\n");
		return -EINVAL;
	}

	/* dump gpled registers */
	qpnp_dump_regs(led, gpled_debug_regs, ARRAY_SIZE(gpled_debug_regs));
...
static int __devinit qpnp_rgb_init(struct qpnp_led_data *led)
{
	...
	printk("[+++ LED +++] %s: Enabled RGB IDs = %X\n", __func__, led->id);
...
//++ p11309 - 2013.03.27 for Qualcomm Patch + Modified
	...
	case QPNP_ID_GPLED_2:
	case QPNP_ID_GPLED_3:
		rc = qpnp_gpled_init(led);
		if (rc)
			dev_err(&led->spmi_dev->dev,
				"GPLED initialize failed(%d)\n", rc);
		break;
//-- p11309
	case QPNP_ID_LED_MPP:
...
//++ p11309 - 2013.03.27 for Qualcomm Patch + Modified
static int __devinit qpnp_get_config_gpled(struct qpnp_led_data *led,
				struct device_node *node)
{
	...
	led->gpled_cfg = devm_kzalloc(&led->spmi_dev->dev,
				sizeof(struct gpled_config_data), GFP_KERNEL);
	if (!led->gpled_cfg) {
		dev_err(&led->spmi_dev->dev, "[+++ LED +++] GPLED: Unable to allocate memory\n");
		return -ENOMEM;
	}	

	rc = of_property_read_u32(node, "qcom,mode", &val);
	if (!rc)
		led->gpled_cfg->mode = (u8) val;
	else {
		dev_err(&led->spmi_dev->dev, "[+++ LED +++] GPLED: qcom,mode\n");
		return rc;
	}

	rc = of_property_read_u32(node, "qcom,pwm-channel", &val);
	if (!rc)
		led->gpled_cfg->pwm_channel = (u8) val;
	else {
		dev_err(&led->spmi_dev->dev, "[+++ LED +++] GPLED: qcom,pwm-channel\n");
		return rc;
	}

	rc = of_property_read_u32(node, "qcom,pwm-us", &val);
	if (!rc)
		led->gpled_cfg->pwm_period_us = val;
	else {
		dev_err(&led->spmi_dev->dev, "[+++ LED +++] GPLED: qcom,pwm-us\n");
		return rc;
	}

	if (led->gpled_cfg->mode == GPLED_MODE_LPG) {
		led->gpled_cfg->duty_cycles =
			devm_kzalloc(&led->spmi_dev->dev,
			sizeof(struct pwm_duty_cycles), GFP_KERNEL);
		if (!led->gpled_cfg->duty_cycles) {
			dev_err(&led->spmi_dev->dev,"[+++ LED +++] Unable to allocate memory\n");
			return -ENOMEM;
		}

		rc = of_property_read_u32(node, "qcom,duty-ms", &val);
		if (!rc)
			led->gpled_cfg->duty_cycles->duty_ms = (u8) val;
		else {
			dev_err(&led->spmi_dev->dev, "[+++ LED +++] GPLED: qcom,duty-ms\n");
			return rc;
		}

		prop = of_find_property(node, "qcom,duty-pcts",
			&led->gpled_cfg->duty_cycles->num_duty_pcts);

		led->gpled_cfg->duty_cycles->num_duty_pcts /= sizeof(u32);

		if (!prop) {
			dev_err(&led->spmi_dev->dev, "[+++ LED +++] qcom,duty-pcts: Looking up property " \
				"node qcom,duty-pcts failed\n");
			return -ENODEV;
		} else if (!led->gpled_cfg->duty_cycles->num_duty_pcts) {
			dev_err(&led->spmi_dev->dev, "[+++ LED +++] qcom,duty-pcts: Invalid length of " \
				"duty pcts\n");
			return -EINVAL;
		}
		...
		led->gpled_cfg->duty_cycles->duty_pcts =
			devm_kzalloc(&led->spmi_dev->dev,
			sizeof(int) * led->gpled_cfg->duty_cycles->num_duty_pcts,
			GFP_KERNEL);

		if (!led->gpled_cfg->duty_cycles->duty_pcts) {
			dev_err(&led->spmi_dev->dev, "[+++ LED +++] qcom,duty-pcts: Unable to allocate memory\n");
			return -ENOMEM;
		}
		...
		if (!temp_cfg) {
			dev_err(&led->spmi_dev->dev, "[+++ LED +++] qcom,duty-pcts: Failed to allocate memory for duty pcts\n");
			return -ENOMEM;
		}		

		rc = of_property_read_u32_array(node, "qcom,duty-pcts",
			temp_cfg, 
			led->gpled_cfg->duty_cycles->num_duty_pcts
		);

		if (!rc) {
			for (i = 0; i < led->gpled_cfg->duty_cycles->num_duty_pcts; i++) {
				led->gpled_cfg->duty_cycles->duty_pcts[i] = (int) temp_cfg[i];
			//	printk("[+++ LED +++] GPLED: %s, %d: %d\n", __func__, i, (int) temp_cfg[i]);
			}
		}
		else {
			dev_err(&led->spmi_dev->dev, "[+++ LED +++] GPLED: qcom,duty-pcts\n");
			return rc;
		}

		rc = of_property_read_u32(node, "qcom,start-idx", &val);
		if (!rc) {
			led->gpled_cfg->lut_params.start_idx = (u8) val;
			led->gpled_cfg->duty_cycles->start_idx = (u8) val;
		} 
		else {
			dev_err(&led->spmi_dev->dev, "[+++ LED +++] GPLED: qcom,start-idx\n");
			return rc;
		}

		led->gpled_cfg->lut_params.idx_len =
			led->gpled_cfg->duty_cycles->num_duty_pcts;
		led->gpled_cfg->lut_params.lut_pause_hi = 0;
		led->gpled_cfg->lut_params.lut_pause_lo = 0;
		led->gpled_cfg->lut_params.ramp_step_ms = led->gpled_cfg->duty_cycles->duty_ms;
		led->gpled_cfg->lut_params.flags = QPNP_LED_PWM_FLAGS;
	}
...
#ifdef CONFIG_F_SKYDISP_SPLASH_WLED_BUGFIX			
			led->default_on = true;
...
//++ p11309 - 2013.03.27 for Qualcomm Patch + Modified
		} else if (strncmp(led_label, "gpled", sizeof("gpled")) == 0) {

			rc = qpnp_get_config_gpled(led, temp);
			if (rc < 0) {
				dev_err(&led->spmi_dev->dev,
					"(%s), (%s) Unable to read gpled config data\n", led_label, led->cdev.name);
				goto fail_id_check;
			}
			
			gpled_enabled_port |= (0x1 << (led->id - QPNP_ID_GPLED_0));
//-- p11309
