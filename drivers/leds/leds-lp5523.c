...
{
	int rc=0,i;
	//return false because led ic can't control medium value 
	return rc;
	for(i=0;i<PAN_NUM_LEDS;i++){
		if(i == EF59_MENU_KEY || i == EF59_BACK_KEY) continue;
...
#else		
	schedule_work(&control_led->brightness_work);
#endif
	return;
}
		...
		enum led_brightness brightness)
{
  ...
  queue_work(pan_led_single_thread_queue, &led->brightness_work);
#else
    ...
    case DIMMING_OFF_CHECK:
      ...
      mod_timer(&led_off_check_timer, jiffies + msecs_to_jiffies(LED_OFF_CHECK_INTERVAL));  
		...
		case DIMMING_ON_COLOR_30S:
			...
			if(check_min_max_value()){
				pan_dbg("DIMMING_ON_COLOR(2).\n");

				if(chip->leds[0].brightness != chip->leds[2].brightness){
					ret = chip->leds[0].brightness - chip->leds[2].brightness ;
					pan_dbg("DIMMING_ON_COLOR(2) green [0]brightness - [2]brightness -> %d\n",ret);
					if(ret > 0){
						if(chip->leds[2].brightness > 0){
							lp5523_write(pan_led->client, LP5523_REG_LED_PWM_BASE    ,chip->leds[2].brightness);
							lp5523_write(pan_led->client, LP5523_REG_LED_PWM_BASE + 2 ,chip->leds[2].brightness);
						}
						if(led->brightness == DIMMING_ON_COLOR_10S){
							pan_pattern[3][1]=0x05;
							pan_pattern[3][3]=0x00;
							pan_pattern[3][5]=ret;
							pan_pattern[3][9]=ret;
							ret = lp5523_load_program(&chip->engines[0], pan_pattern[3]);
						}else if(led->brightness == DIMMING_ON_COLOR_20S){
							pan_pattern[4][1]=0x05;
							pan_pattern[4][3]=0x00;
							pan_pattern[4][5]=ret;
							pan_pattern[4][9]=ret;
							ret = lp5523_load_program(&chip->engines[0], pan_pattern[4]);
						}else if(led->brightness == DIMMING_ON_COLOR_30S){
							pan_pattern[5][1]=0x05;
						...
						first_color|=LP5523_CMD_RUN_ENGINE1;
						if(ret)
							pan_err("DIMMING_ON_COLOR Ramp ON error ret -> %d LINE -> %d\n",ret,__LINE__);
						...
						if(led->brightness == DIMMING_ON_COLOR_10S){
							pan_pattern[14][1]=0x05;
							pan_pattern[14][3]=0x00;
							pan_pattern[14][5]=(-ret);
							pan_pattern[14][9]=(-ret);
							ret = lp5523_load_program(&chip->engines[0], pan_pattern[14]);
						}else if(led->brightness == DIMMING_ON_COLOR_20S){
							pan_pattern[15][1]=0x05;
							pan_pattern[15][3]=0x00;
							pan_pattern[15][5]=(-ret);
							pan_pattern[15][9]=(-ret);
							ret = lp5523_load_program(&chip->engines[0], pan_pattern[15]);
						}else if(led->brightness == DIMMING_ON_COLOR_30S){
							pan_pattern[16][1]=0x05;
							pan_pattern[16][3]=0x00;
							pan_pattern[16][5]=(-ret);
							pan_pattern[16][9]=(-ret);
							ret = lp5523_load_program(&chip->engines[0], pan_pattern[16]);
						}else{
							break;
						}
						first_color|=LP5523_CMD_RUN_ENGINE1;
					...
					pan_dbg("DIMMING_ON_COLOR(2) green brightness %d\n",chip->leds[0].brightness);
					if(chip->leds[0].brightness > 0){
						lp5523_write(pan_led->client, LP5523_REG_LED_PWM_BASE  ,chip->leds[0].brightness);
						lp5523_write(pan_led->client, LP5523_REG_LED_PWM_BASE + 2,chip->leds[2].brightness);
					}          
				}

				if(chip->leds[1].brightness != chip->leds[3].brightness){
					ret = chip->leds[1].brightness - chip->leds[3].brightness ;
					pan_dbg("DIMMING_ON_COLOR(2) blue [1]brightness - [3]brightness -> %d\n",ret);
					if(ret > 0){
						if(chip->leds[3].brightness > 0){
							lp5523_write(pan_led->client, LP5523_REG_LED_PWM_BASE + 1 ,chip->leds[3].brightness);
							lp5523_write(pan_led->client, LP5523_REG_LED_PWM_BASE + 3 ,chip->leds[3].brightness);
						}
						if(led->brightness == DIMMING_ON_COLOR_10S){
							pan_pattern[3][1]=0x0a;
							pan_pattern[3][3]=0x10;
							pan_pattern[3][5]=ret;
							pan_pattern[3][9]=ret;
							ret = lp5523_load_program(&chip->engines[1], pan_pattern[3]);
						}else if(led->brightness == DIMMING_ON_COLOR_20S){
							pan_pattern[4][1]=0x0a;
							pan_pattern[4][3]=0x10;
							pan_pattern[4][5]=ret;
							pan_pattern[4][9]=ret;
							ret = lp5523_load_program(&chip->engines[1], pan_pattern[4]);
						}else if(led->brightness == DIMMING_ON_COLOR_30S){
							pan_pattern[5][1]=0x0a;
							pan_pattern[5][3]=0x10;
							pan_pattern[5][5]=ret;
							pan_pattern[5][9]=ret;
							ret = lp5523_load_program(&chip->engines[1], pan_pattern[5]);
						}else{
							break;
						}
						first_color|=LP5523_CMD_RUN_ENGINE2;
						...
						if(led->brightness == DIMMING_ON_COLOR_10S){
							pan_pattern[14][1]=0x0a;
							pan_pattern[14][3]=0x10;
							pan_pattern[14][5]=(-ret);
							pan_pattern[14][9]=(-ret);
							ret = lp5523_load_program(&chip->engines[1], pan_pattern[14]);
						}else if(led->brightness == DIMMING_ON_COLOR_20S){
							pan_pattern[15][1]=0x0a;
							pan_pattern[15][3]=0x10;
							pan_pattern[15][5]=(-ret);
							pan_pattern[15][9]=(-ret);
							ret = lp5523_load_program(&chip->engines[1], pan_pattern[15]);
						}else if(led->brightness == DIMMING_ON_COLOR_30S){
							pan_pattern[16][1]=0x0a;
							pan_pattern[16][3]=0x10;
							pan_pattern[16][5]=(-ret);
							pan_pattern[16][9]=(-ret);
							ret = lp5523_load_program(&chip->engines[1], pan_pattern[16]);
						}else{
							break;
						}
						first_color|=LP5523_CMD_RUN_ENGINE2;
					...
					pan_dbg("DIMMING_ON_COLOR(2) blue brightness %d\n",chip->leds[1].brightness);
					if(chip->leds[1].brightness > 0){
						lp5523_write(pan_led->client, LP5523_REG_LED_PWM_BASE + 1,chip->leds[1].brightness);
						lp5523_write(pan_led->client, LP5523_REG_LED_PWM_BASE + 3,chip->leds[3].brightness);
					}          
				}

				if(chip->leds[6].brightness != chip->leds[7].brightness){
					ret = chip->leds[6].brightness - chip->leds[7].brightness ;
					pan_dbg("DIMMING_ON_COLOR(2) red [6]brightness - [7]brightness -> %d\n",ret);
					if(ret > 0){
						if(chip->leds[7].brightness > 0){
							lp5523_write(pan_led->client, LP5523_REG_LED_PWM_BASE + 6 ,chip->leds[7].brightness);
							lp5523_write(pan_led->client, LP5523_REG_LED_PWM_BASE + 7 ,chip->leds[7].brightness);
						}
						if(led->brightness == DIMMING_ON_COLOR_10S){
							pan_pattern[3][1]=0xc0;
							pan_pattern[3][3]=0x20;
							pan_pattern[3][5]=ret;
							pan_pattern[3][9]=ret;
							ret = lp5523_load_program(&chip->engines[2], pan_pattern[3]);
						}else if(led->brightness == DIMMING_ON_COLOR_20S){
							pan_pattern[4][1]=0xc0;
							pan_pattern[4][3]=0x20;
							pan_pattern[4][5]=ret;
							pan_pattern[4][9]=ret;
							ret = lp5523_load_program(&chip->engines[2], pan_pattern[4]);
						}else if(led->brightness == DIMMING_ON_COLOR_30S){
							pan_pattern[5][1]=0xc0;
							pan_pattern[5][3]=0x20;
							pan_pattern[5][5]=ret;
							pan_pattern[5][9]=ret;
							ret = lp5523_load_program(&chip->engines[2], pan_pattern[5]);
						}else{
							break;
						}
						first_color|=LP5523_CMD_RUN_ENGINE3;
						...
						if(led->brightness == DIMMING_ON_COLOR_10S){
							pan_pattern[14][1]=0xc0;
							pan_pattern[14][3]=0x20;
							pan_pattern[14][5]=(-ret);
							pan_pattern[14][9]=(-ret);
							ret = lp5523_load_program(&chip->engines[2], pan_pattern[14]);
						}else if(led->brightness == DIMMING_ON_COLOR_20S){
							pan_pattern[15][1]=0xc0;
							pan_pattern[15][3]=0x20;
							pan_pattern[15][5]=(-ret);
							pan_pattern[15][9]=(-ret);
							ret = lp5523_load_program(&chip->engines[2], pan_pattern[15]);
						}else if(led->brightness == DIMMING_ON_COLOR_30S){
							pan_pattern[16][1]=0xc0;
							pan_pattern[16][3]=0x20;
							pan_pattern[16][5]=(-ret);
							pan_pattern[16][9]=(-ret);
							ret = lp5523_load_program(&chip->engines[2], pan_pattern[16]);
						}else{
							break;
						}
						first_color|=LP5523_CMD_RUN_ENGINE3;
					...
					pan_dbg("DIMMING_ON_COLOR(2) red brightness %d\n",chip->leds[6].brightness);
					if(chip->leds[1].brightness > 0){
						lp5523_write(pan_led->client, LP5523_REG_LED_PWM_BASE + 6,chip->leds[6].brightness);
						lp5523_write(pan_led->client, LP5523_REG_LED_PWM_BASE + 7,chip->leds[7].brightness);
					}          
				}
				if(first_color > 0){
