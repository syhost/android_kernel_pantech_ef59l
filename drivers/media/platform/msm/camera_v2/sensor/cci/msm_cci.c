...
#define CONFIG_PANTECH_CAMERA_EEPROM
#endif
...
#ifdef CONFIG_PANTECH_CAMERA_EEPROM
static int32_t msm_cci_i2c_read_eeprom(struct v4l2_subdev *sd,
	struct msm_camera_cci_ctrl *c_ctrl, uint16_t eeprom_sid)//, uint16_t data_addr)
{
	uint32_t rc = 0;
	uint32_t val = 0;
	...
	CDBG("%s set param sid 0x%x retries %d id_map %d\n", __func__,
		eeprom_sid, c_ctrl->cci_info->retries,
		c_ctrl->cci_info->id_map);
	CDBG("%s read_cfg->addr 0x%x \n", __func__,read_cfg->addr);

	val = CCI_I2C_SET_PARAM_CMD | eeprom_sid << 4 |
	...
	CDBG("%s read_cfg->addr_type = %d, read_cfg->addr =0x%x \n", __func__,(int)read_cfg->addr_type, read_cfg->addr);
	...
	CDBG("%s:%d read_words = %d, exp words = %d\n", __func__,
	...
	struct msm_camera_cci_ctrl *c_ctrl)
{
...
#ifdef CONFIG_PANTECH_CAMERA_EEPROM
	uint16_t eeprom_sid = 0;
	uint16_t remaind_addr = 0;
	uint16_t portion_addr = 0;
	uint16_t page_addr = 256;
#endif
	...
	do {
#ifdef CONFIG_PANTECH_CAMERA_EEPROM
		CDBG("%s:%d sid = 0x%x, addr = 0x%x\n", __func__, __LINE__,c_ctrl->cci_info->sid<<1, read_cfg->addr);
		if(c_ctrl->cci_info->sid<<1 == 0xA0 && read_cfg->addr != 0) {
			remaind_addr= read_cfg->addr % page_addr;
			portion_addr = (int)(read_cfg->addr/page_addr);//(int)((read_cfg->addr-1)/page_addr);
			CDBG("%s:%d remaind_addr = %d, portion_addr = 0x%x\n", __func__, __LINE__, remaind_addr, portion_addr);

			eeprom_sid = c_ctrl->cci_info->sid + portion_addr;

			if(remaind_addr > page_addr-CCI_READ_MAX) {
				read_cfg->num_byte = page_addr - remaind_addr;
				pr_err("%s : 1 read_bytes = %d, remaind_addr = %d, portion_addr=%d, num_byte = %d, addr = %d",__func__, read_bytes, remaind_addr, portion_addr, read_cfg->num_byte, read_cfg->addr);
			}
			else {
				if (read_bytes > CCI_READ_MAX) {
					read_cfg->num_byte = CCI_READ_MAX;
					pr_err("%s : 2 read_bytes = %d, remaind_addr = %d, portion_addr=%d, num_byte = %d, addr = %d",__func__, read_bytes, remaind_addr, portion_addr, read_cfg->num_byte, read_cfg->addr);
				}
				else {
					read_cfg->num_byte = read_bytes;
					pr_err("%s : 3 read_bytes = %d, remaind_addr = %d, portion_addr=%d, num_byte = %d, addr = %d",__func__, read_bytes, remaind_addr, portion_addr, read_cfg->num_byte, read_cfg->addr);
				}
			}
			//if(read_cfg->addr == 762) eeprom_sid -= 1;
			CDBG("%s:%d eeprom_sid = %x, read_bytes = %d, num_byte = %d\n", __func__, __LINE__, eeprom_sid, read_bytes, read_cfg->num_byte);
			rc = msm_cci_i2c_read_eeprom(sd, c_ctrl, eeprom_sid);//, read_cfg->addr);//remaind_addr+(page_addr*portion_addr));
			if (rc < 0) {
				pr_err("%s:%d failed rc %d\n", __func__, __LINE__, rc);
				goto ERROR;
			}
			
			CDBG("%s:%d read_bytes = %d\n", __func__, __LINE__, read_bytes);
			if (read_bytes > 0) {
				read_cfg->addr += read_cfg->num_byte;
				read_cfg->data += read_cfg->num_byte;
				read_bytes -= read_cfg->num_byte;
			} else {
				read_bytes = 0;
			}			
