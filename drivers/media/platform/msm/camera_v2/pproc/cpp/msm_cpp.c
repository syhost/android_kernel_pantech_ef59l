...
static int cpp_init_hardware(struct cpp_device *cpp_dev)
{
...
#ifdef CONFIG_PANTECH_CAMERA  // patch for case #01393584, 2013-12-20
	cpp_dev->cpp_clk[MSM_MICRO_IFACE_CLK_IDX] = clk_get(&cpp_dev->pdev->dev, cpp_clk_info[MSM_MICRO_IFACE_CLK_IDX].clk_name); 
	...
	rc = clk_reset(cpp_dev->cpp_clk[MSM_MICRO_IFACE_CLK_IDX], CLK_RESET_ASSERT); 
	if (rc) { 
	...
	rc = clk_reset(cpp_dev->cpp_clk[MSM_MICRO_IFACE_CLK_IDX], CLK_RESET_DEASSERT); 
	if (rc) { 
		pr_err("%s:micro_iface_clk assert failed\n", __func__); 
		clk_put(cpp_dev->cpp_clk[MSM_MICRO_IFACE_CLK_IDX]); 
		goto remap_failed; 
