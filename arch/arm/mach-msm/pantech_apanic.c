...
#define MAX_CORES 4   //goni 20130528
...
extern struct msg_log *get_pantech_logcat_dump_address(void);
...
static void apanic_get_mmu_info(struct panic_info *info)
{
	...
	WRITE_LOG("model name : %s\n",p_panic_info->phoneinfo_buff_ptr.model_name_);
	WRITE_LOG("sw version : %s\n",p_panic_info->phoneinfo_buff_ptr.binary_version_);
	WRITE_LOG("build date : %s\n",p_panic_info->phoneinfo_buff_ptr.build_date_);
	WRITE_LOG("build time : %s\n	 ",p_panic_info->phoneinfo_buff_ptr.build_time_);
	WRITE_LOG("\n\n"); 
...
void pantech_force_dump_key(unsigned int code, int value)
{
	...
	//goni 20130523 printk(KERN_DEBUG "Crash key : %d , value : %d\n", code, value);
...
void pantech_pm_log(int percent_soc, unsigned int cable_type)
{
	...
	if( (p_panic_info->usbdump == 0) && (p_panic_info->ramdump == 0) )
		return;
...
int pantech_is_usbdump_enabled(void)
{
	...
	return p_panic_info->usbdump;
}
EXPORT_SYMBOL(pantech_is_usbdump_enabled);

#define PRINT_MSG_BUF_SIZE 20
void pantech_schedule_log(enum sched_type sched, int cpu, struct task_struct *task, char *msg)
{
	unsigned i;
	char print_msg[PRINT_MSG_BUF_SIZE];
	...
	switch(sched){
		case MAIN_SCHED :
			break;
		case WORKQUEUE_SCHED :
			strcpy(print_msg, " WQ:");
			snprintf(print_msg+4, PRINT_MSG_BUF_SIZE-4, "%pS", msg);
			break;
		case PREEMPT_SCHED_IRQ :
			strcpy(print_msg, " PRMPT SCHED IRQ");
			break;
		default :
			break;
	}

	i = atomic_inc_return(&(p_panic_info->sched_idx[cpu]))
		& (SCHED_LOG_MAX - 1);
	p_panic_info->sched[cpu][i].time = cpu_clock(cpu);
	if (task) {
		strncpy(p_panic_info->sched[cpu][i].comm, task->comm,
			sizeof(p_panic_info->sched[cpu][i].comm));
		p_panic_info->sched[cpu][i].pid = task->pid;
	} else {
		strncpy(p_panic_info->sched[cpu][i].comm, print_msg,
			sizeof(p_panic_info->sched[cpu][i].comm));
		p_panic_info->sched[cpu][i].pid = -1;
	}
}

void pantech_irq_log(unsigned int softirq_vec, unsigned int func, unsigned long long start_time)
{
	...
	i = atomic_inc_return(&(p_panic_info->irq_idx[cpu]))
		& (SCHED_LOG_MAX - 1);

	p_panic_info->irq[cpu][i].softirq_vec = softirq_vec;
	p_panic_info->irq[cpu][i].func = func;
	p_panic_info->irq[cpu][i].start_time = start_time;
	p_panic_info->irq[cpu][i].elapsed_time = cpu_clock(cpu) - start_time;
	p_panic_info->irq[cpu][i].irqs_flag = irqs_disabled();
	}

void pantech_phone_info_log(void *phone_info)
{
	memcpy(&p_panic_info->phoneinfo_buff_ptr, phone_info, sizeof(phoneinfo_type));
}
                  ...
                  void *ptr)
{
	...
	}else if(!strncmp(ptr, "adsp", 4)){
		sky_reset_reason=SYS_RESET_REASON_ADSP;
	}else if(!strncmp(ptr, "venus", 5)){
		sky_reset_reason=SYS_RESET_REASON_VENUS;
	}
...
int __init apanic_pantech_init(void)
{
	...
	int i;
    struct msg_log *temp_logcat_header;
	...
	*/
	...
	p_panic_info = ioremap_nocache(PANTECH_SHARED_RAM_BASE, SZ_512K);
	
	memset(p_panic_info, 0x00, sizeof(struct pantech_panic_info));

	__raw_writel(PANTECH_INFO_MAGIC, &p_panic_info->magic);
	__raw_writel(virt_to_phys(get_log_buf_addr()), &p_panic_info->msg.kernellog_buf_adr);

    // (+) p16652 add for logcat parser
    temp_logcat_header = get_pantech_logcat_dump_address();
    p_panic_info->msg.mainlogcat_buf_adr = temp_logcat_header->mainlogcat_buf_adr;
    p_panic_info->msg.mainlogcat_w_off = temp_logcat_header->mainlogcat_w_off;
    p_panic_info->msg.mainlogcat_size = temp_logcat_header->mainlogcat_size;

    p_panic_info->msg.systemlogcat_buf_adr = temp_logcat_header->systemlogcat_buf_adr;
    p_panic_info->msg.systemlogcat_w_off = temp_logcat_header->systemlogcat_w_off;
    p_panic_info->msg.systemlogcat_size = temp_logcat_header->systemlogcat_size;

	/*
	__raw_writel(virt_to_phys(logcat_get_addr(1)), &p_panic_info->msg.mainlogcat_buf_adr);
	__raw_writel(virt_to_phys(logcat_get_addr(4)), &p_panic_info->msg.systemlogcat_buf_adr);
    */

	for (i = 0; i < CONFIG_NR_CPUS; i++) {
		atomic_set(&(p_panic_info->sched_idx[i]), -1);
		atomic_set(&(p_panic_info->irq_idx[i]), -1);
	}
