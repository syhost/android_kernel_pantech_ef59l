...
//#define LCD_CLK_DEBUG

#ifdef LCD_CLK_DEBUG 
#define EXT_NUM 14
int except_str_cmp(const char *clk_name) {
	char *except_clks[EXT_NUM] = {"mmss_s0_axi_clk", "cxo_clk_src", "bimc_msmbus_clk", "gpll0_clk_src", "mmpll1_clk_src", "ocmemnoc_clk", "ocmemgx_clk", "snoc_msmbus_clk", "cnoc_msmbus_a_clk", "cnoc_msmbus_clk",
		"pnoc_msmbus_a_clk", "snoc_msmbus_a_clk", "bimc_msmbus_a_clk", "gcc_prng_ahb_clk"} ; 
	int i = 0; 
	int ret_val = 0;
	
	for(i = 0; i < EXT_NUM; i++) 
	 if(!strcmp(clk_name, except_clks[i])) 
	 ret_val = 1;
...
#ifdef LCD_CLK_DEBUG 
	if(!except_str_cmp(clk->dbg_name)) 
	 printk("%s: %s - cnt: %d\n", __func__, clk->dbg_name, clk->prepare_count); 
#endif
...
#ifdef LCD_CLK_DEBUG 
	if(!except_str_cmp(clk->dbg_name)) 
	 printk("%s: %s - cnt: %d\n", __func__, clk->dbg_name, clk->count); 
#endif
	if (clk->count == 0) {
		parent = clk->parent;
...
static int __init __handoff_clk(struct clk *clk)
{
	...
	//printk("[QCT_TEST] %s , %s, state : %d\n",clk->dbg_name, __func__,state); 
