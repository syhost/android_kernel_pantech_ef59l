...
int msm_xo_mode_vote(struct msm_xo_voter *xo_voter, enum msm_xo_modes mode)
{
	...
	pr_err("msm_xo_mode_vote name[%s] mode[%d]", xo_voter->name, mode);
...
struct msm_xo_voter *msm_xo_get(enum msm_xo_ids xo_id, const char *voter)
{
	...
	pr_info("msm_xo_get name[%s]", xo_voter->name);
	if (!xo_voter->name) {
		ret = -ENOMEM;
		goto err_name;
	}
	...
	pr_info("msm_xo_get mode[%x]", xo_voter->xo->mode);
