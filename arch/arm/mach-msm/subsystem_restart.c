...
static int modem_ssr_dump_enable = 1;
// sync with vendor/pantech/frameworks/testmenu_server/inc/skytest.h
#define SSR_SET_MODEM_DISABLE_INDEX 0
#define SSR_SET_MODEM_DUMP_INDEX 1
#define SSR_SET_MODEM_CHECK(b)       ((unsigned char)1 == b[SSR_SET_MODEM_DISABLE_INDEX])
#define SSR_SET_MODEM_DUMP_CHECK(b)  ((unsigned char)1 == b[SSR_SET_MODEM_DUMP_INDEX])
int rawdata_read_func(unsigned int offset, unsigned int read_size, char* read_buf);
void check_ssr_setting_func( struct work_struct *_work );
#endif
...
// p15060
	...
	for_each_subsys_device(list, count, NULL, subsystem_ramdump);
        ...
        if(!strncmp(desc->name, "modem", 5)){
                sys_sync();
                panic(desc->name, "subsys-restart: Resetting the SoC - %s crashed.", desc->name);
        }
#endif 
...
void check_ssr_setting_func( struct work_struct *_work )
{
    ...
    if( rawdata_read_func(SSR_SETTING_BACKUP_START, sizeof(read_buf), read_buf) >= 0 )
    {
        if( SSR_SET_MODEM_CHECK( read_buf ) )
        {
            modem_ssr_enable = 0;
            pr_err( "Modem SSR disable.\n" );
        }
        else
        {
            modem_ssr_enable = 1;
            pr_err( "Modem SSR enable.\n" );
        }
        
        if( SSR_SET_MODEM_DUMP_CHECK( read_buf ) )
        {
            modem_ssr_dump_enable = 1;
            pr_err( "Modem SSR dump enabled.\n" );
        }
        else
        {
            modem_ssr_dump_enable = 0;
            pr_err( "Modem SSR dump disabled.\n" );
        }
    }
    else
    {
...
#ifdef CONFIG_PANTECH_ERR_CRASH_LOGGING
// p15060
    if( (!strncmp(name, "modem", 5) && pantech_is_usbdump_enabled()) ||
        (!strncmp(name, "modem", 5) && (0 == modem_ssr_enable))      )
    {
        pr_err( "dev->restart_level = RESET_SOC\n" );
...
// p15060
	...
	case RESET_SOC:
		#ifdef CONFIG_PANTECH_ERR_CRASH_LOGGING
		panic(name, "subsys-restart: Resetting the SoC - %s crashed.", name);
		#else
...
irqreturn_t subsys_err_ready_intr_handler(int irq, void *subsys)
{
