...
extern void check_ssr_setting_func( struct work_struct *_work );
struct ssr_setting_work {
    struct work_struct work;
};
struct ssr_setting_work *ssr_work;
#endif
...
// p15060
#ifdef CONFIG_PANTECH_ERR_CRASH_LOGGING
            ssr_work = kmalloc(sizeof *ssr_work, GFP_KERNEL);
            INIT_WORK(&ssr_work->work, check_ssr_setting_func);
            schedule_work( &ssr_work->work );
#endif
