...
static void __init cpufreq_table_init(void)
{
    ...
    int chargerfreq;
		for (i = 0, freq_cnt = 0; drv.acpu_freq_tbl[i].speed.khz != 0
            ...
            chargerfreq = 0; 
	  dev_dbg(drv.dev, " cpu(%d) power on(%d) khz (%ld)\n", cpu,oem_smem_boot_mode_read(), drv.acpu_freq_tbl[i].speed.khz);
        if (oem_smem_boot_mode_read() || drv.acpu_freq_tbl[i].speed.khz <= 960000) {
		dev_dbg(drv.dev, "chargerfreq (%d) khz (%ld)\n", chargerfreq, drv.acpu_freq_tbl[i].speed.khz);
            chargerfreq = 1; 
        }
...
//p16101 for PVS
    ...
    len  = sprintf(page, "SPEED BIN : 0x%x , PVS : 0x%x", drv.speed_bin, drv.pvs_bin);
//p16101 for PVS
