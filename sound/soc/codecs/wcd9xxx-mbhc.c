...
// LS1@SND : Write headset button ADC level for test menu #8378522# Earphone Key Test
...
/* 
 * 20130811 LS1@SND - Workaround code for delay 3-pole headset event report to avoid wrong vdces level issue related to earjack socket problem.
 * CASE#01244567 - [MSM8974] Requirements related to the polling noise of headset.
 */
#ifdef CONFIG_PANTECH_SND
#define HEADPHONE_REPORT_FORCE_SKIP_MAX_NUM 5
int find_plug_type_attempt_cnt = 0;
#endif /* CONFIG_PANTECH_SND */
...
#if defined(CONFIG_PANTECH_SND_BOOTUP_HEADSET_INFO)
				     ...
				     bool override_bypass, bool noreldetection)
{
    ...
    /* Turn off MBHC clock prior to MBHC reset for better ADC accuracy */
...
#ifdef CONFIG_PANTECH_SND_QCOM_PATCH // 20131111 hdj iphone heaset is not deceted issue. 
	int highhph_cnt = 0;
#endif
 ...
 */
	...
	if (type == PLUG_TYPE_HEADPHONE && find_plug_type_attempt_cnt < HEADPHONE_REPORT_FORCE_SKIP_MAX_NUM) {
		pr_debug("%s: [VEGA] Set force PLUG_TYPE_HEADPHONE to PLUG_TYPE_INVALID, Count : %d\n", __func__, find_plug_type_attempt_cnt);
		type = PLUG_TYPE_INVALID;
	}
#endif /* CONFIG_PANTECH_SND */
exit:
...
/* 
 * 2013-09-11 LS1@SND - QCOM workaround for car hansfree cable recognition error issue.
...
static void create_hs_button_testmenu_entries(void)
{
	...
	if (wcd9320_headset_button_dir == NULL) {
		pr_err("Unable to create /proc/taiko_codec directory\n");
	}
...
/* 2013-06-13 LS1@SND Fixed for Headset Volume up/down Key delay */
#ifdef CONFIG_PANTECH_SND
