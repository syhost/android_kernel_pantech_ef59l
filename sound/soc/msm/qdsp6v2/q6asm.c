		...
		uint32_t format)
{
	...
	pr_debug("%s: topology id = 0x%x, get_asm_topology():0x%08x\n", __func__, open.preprocopo_id, get_asm_topology());
		...
		uint16_t bits_per_sample)
{
...
#ifdef CONFIG_PANTECH_SND_QSOUND //from qsound
	if(open.bits_per_sample ==16){
	...
	if (open.postprocopo_id == 0)
        ...
        pr_debug("%s, open.postprocopo_id:0x%08x, get_asm_topology():0x%08x\n", __func__, open.postprocopo_id, get_asm_topology());
			...
			uint32_t wr_format)
{
	...
	if (open.postprocopo_id == 0)
        ...
        pr_debug("%s, open.postprocopo_id:0x%08x, get_asm_topology():0x%08x", __func__, open.postprocopo_id, get_asm_topology());
